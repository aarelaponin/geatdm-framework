...
- [ ] **On-screen practice box on the recap slide.** Every subtopic carries a `practice` field naming the artefact its AI tip produces. It renders as an un-narrated box on the recap slide, built from **two** strings taken verbatim from the AI tip — the tip's `title` (the task) and the `practice` field (the artefact): **Do this on your own sector. [tip title].** The prompt in the companion material gives you [artefact]. Before the next video. The artefact alone is the Output half of an I/O spec, so it names the result's *shape* ("a structured ToR document") and reads as a generic call to action — the task line is what makes it specific. Say "companion material", never "the description" or a named platform: where the prompt lives differs per channel, and the slide is cut once. It replaces the narrated "Your play" handoff, so nothing in the voice-over may mention the prompt, the companion material or the listener's own sector.
...
5. **On-screen practice box** — The `practice` field: the artefact string, matching the AI tip's inputs-and-outputs line so the two cannot drift. It is paired on the slide with the tip's `title`, which supplies the subject the artefact string lacks. Renders on the recap slide, never narrated.
