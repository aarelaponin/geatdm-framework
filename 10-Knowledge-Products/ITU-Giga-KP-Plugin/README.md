...
| `kp-whisper-transcribe` | **Mechanic** | Transcribe a narration take (`.m4a`) to the `.srt` that `kp-audio-brief` Step 6 audits, using local `openai-whisper` — no upload, no API key. Covers this Mac's install and certificate failure modes. Run whenever a fresh take lands in a module's `audio/` folder with no matching `.srt`. |
