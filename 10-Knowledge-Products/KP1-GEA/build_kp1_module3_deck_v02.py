#!/usr/bin/env python3
# Build the KP1 Module 3 (Topic 3) video deck on the ITU template — v0.2.
# Content follows KP1_Module3_Script_Bundle_v0.2 (build_kp1_module3_v02.js): seven videos,
# the tightened Tier 2 script — the shared-object / shared-rhythm paragraph kept once (3.4),
# one sentence in 3.1; 3.5 keeps only the gate moment; the 3.7 recap capped. Slide counts
# are unchanged from v0.1; every voice-over is the v0.2 beat text, verbatim, one beat per
# `VO:` paragraph. Each video's recap slide carries the on-screen practice box (plan D5) in
# place of the narrated handoff.
...
# The recap slide's notes open with this line. The box is on screen only; nothing in the
# narration reads it, paraphrases it or points at it (plan D5, vo_diff.py enforces it).
...
edit_cover(
    ...
    length='~29 mins across 7 videos (3.1 – 3.7)',
    audience=AUDIENCE,
...
# v0.2: the shared-object paragraph is kept in full in 3.4 (the shared rhythm); here it is one
# sentence, so the v0.1 two-column slide becomes the cue's two text blocks.
block_slide(prs, 'The repository is the object both sides point at',
            ['When the minister briefs cabinet and the architect designs a system, they look at '
             'the same picture — because there is only one.'],
            'The repository is what makes the architecture a shared language instead of two private ones.',
            T,
            "VO: The repository is also the object both sides point at: when the minister briefs "
            "cabinet and the architect designs a system they are looking at the same picture, which "
            "is what makes the architecture a shared language between the business side and the IT "
            "side instead of two private ones.",
            punch_y=3.7)
...
big_slide(prs,
          ...
          "layers, the relationships and the decisions. Let copies multiply, and no amount of "
          "governance will save the architecture.",
          practice=("Design your EA repository's structure",
...
big_slide(prs,
          ...
          T,
          PRACTICE_NOTE + "\n\n"
          "VO: Choose your tooling like any government system: graduate when the spreadsheet truly "
          "hurts, reuse before buy, buy before build, keep your content in open formats you control, "
          "and bend the tool to your metamodel.",
          practice=('Score EA tool options against lock-in and fit',
...
big_slide(prs,
          ...
          T,
          PRACTICE_NOTE + "\n\n"
          "VO: Keep the repository true. One named owner. Updates triggered by events, not a yearly "
          "review. A light conformance check. Neglect it, and within a year you have a "
          "widely-trusted work of fiction.",
          practice=('Draft the repository update policy',
                    'a one-page update policy'))
...
big_slide(prs,
          ...
          T,
          PRACTICE_NOTE + "\n\n"
          "VO: Stand up a Board that can say no: binding, not advisory, chaired by someone senior "
          "enough that the no sticks, made of the people whose systems are affected, on a rhythm "
          "that governs without blocking.",
          practice=("Draft your EA Board's Terms of Reference",
                    'a structured EA Board Terms of Reference'))
...
# v0.2: only the gate moment is kept here — the rational-project preamble is one self-contained
# signpost sentence, so the lead shrinks and the panel lifts toward it.
block_slide(prs, 'The gate is where the whole-of-government view meets the decision',
            ['Inside a project, building your own is faster than reusing, so projects fragment — '
             'and no procurement rule can change that. Only whole-of-government planning makes '
             're-use rational.',
             'The gate is where that view meets the decision, while it can still be changed.'],
            'When a project proposes to build what already exists as a shared block, the Board '
            'says: consume the shared one.',
            T,
            "VO: Here is why this gate matters more than any document. Inside a project, building "
            "your own is faster than reusing, so projects fragment — and no procurement rule can "
            "change that; only whole-of-government planning makes re-use rational. The gate is "
            "where that view meets the decision, while it can still be changed. When a project "
            "proposes to build what already exists as a shared block, the Board says: consume the "
            "shared one.\n\nThe emotional peak of the module. This is the pivotal slide of 3.5 — "
            "let the closing line land before moving on.",
            punch_fill=ITU_BLUE, punch_ink=WHITE, punch_y=4.35)
...
big_slide(prs,
          ...
          practice=('Build your architecture review gate checklist',
                    'an intake checklist, pass criteria, exception form and decision-log fields'))
...
section('3.6', 'Show the EA is working — the few metrics that matter',
        ...
        "The answer — the re-use rate — is delivered two slides on.")
...
big_slide(prs,
          ...
          "made and how fast. On one page, every quarter, amber where they should be amber.",
          practice=('Build your EA health scorecard',
                    'a one-page scorecard template with metrics, computations and thresholds'))
...
section('3.7', 'Keep the practice alive past year two',
        ...
        "delivered on the next four slides, one each.")
...
big_slide(prs,
          ...
          T,
          PRACTICE_NOTE + "\n\n"
          "VO: Watch for the four fades — the team pulled, the repository stale, the Board gone "
          "advisory, the sponsor changed — and counter each. Keep the practice alive past year two, "
          "and the architecture becomes how government works.",
          practice=('Build your EA sustainment risk register',
...
OUT = os.environ.get('OUT_PATH') or os.path.join(
    ...
    'videos', 'module_3', 'en', 'decks', 'KP1_M3_Deck_v0.2.pptx')
prs.save(OUT)
