#!/usr/bin/env python3
# Build the KP1 Module 1 (Topic 1) video deck on the ITU template — v0.2.
# Content follows KP1_Module1_Script_Bundle_v0.3 (build_kp1_module1_v03.js): seven videos,
# the old 1.8 signposts folded into 1.7, the lifecycle video collapsed onto one page.
...
from deck_lib import (
    ...
    open_template, panel, panel_text, practice_box, rows_slide, section_slide, set_text, solid,
    sources_slide, title)
...
for sh in tb_map:
    ...
    if 'Title of the Video' in t:
        ...
        set_text(sh.text_frame, [
            ...
            [('Seven standalone videos: why your country needs a national EA, what it is, why projects cannot fix it, why now, why PAERA, the lifecycle, and the four asks to put to your minister.', 14, False, GREY, False)],
        ], space_after=Pt(6))
    elif 'Lenght' in t or 'Length' in t:
        sh.text_frame.clear()
        set_text(sh.text_frame, [
            [('Seven videos: ', 14, True, INK, False), ('1.1 – 1.7', 14, False, INK, False)],
...
notes(cover, "Cover for the combined Module 1 deck. Each subtopic section that follows is one standalone ~4-5 minute video. The right panel previews the module's centrepiece: the five-phase, four-sign-off lifecycle taught in video 1.6.")
...
for sh in agenda.shapes:
    ...
    if t.strip() == 'Agenda':
        sh.text_frame.clear()
        set_text(sh.text_frame, [[('Module 1 — seven videos', 14, True, GREY, False)]])
...
notes(agenda, 'Navigation slide for the combined deck; the videos themselves ship standalone on YouTube. Videos 1.1-1.4 make the case for an EA, 1.5-1.6 make the case for PAERA and show the method, 1.7 turns conviction into the four commitments — and closes on the four governments that made them.')
...
footer(s, T)
notes(s, "Rows reveal one at a time, cumulative.\n\nRETRIEVAL MOMENT (before revealing row 1): ask the viewer — how many separate registers of the same citizens does your government keep? Hold two seconds, then reveal.\n\nVO: There are four signs that this pattern is happening in your government.\n\nVO: Sign one. Your ministry has more than one register of the same people. The school list has one. The national ID register has another. The social register has a third. Each has its own list. None of them agree.\n\nVO: Sign two. Every time two systems need to share data, you build a new connection from zero. Last year your team connected the tax office to the business register. This year you are doing the same work for the health ministry. The connection is built again, from scratch.\n\nVO: Sign three. A vendor built one of your systems ten years ago. To change anything in it, you must call that vendor and pay what they ask. Nobody else knows how it works. The vendor knows this, and prices accordingly.\n\nVO: Sign four. Each ministry has its own digital systems, and they do not connect across ministries. The citizen who gave her ID number to the health ministry must give it again, on paper, when she enrols her child in school. The pledge to ask each citizen for information only once exists on paper. In practice, it is impossible.")
...
p = panel(s, 0.72, 3.65, 11.8, 1.25, LIGHT)
panel_text(s, 0.72, 3.7, 11.8, 1.25, [
    [('A national Enterprise Architecture is that plan.', 26, True, ITU_BLUE_DARK, False)],
])
practice_box(s, "Diagnose your country's fragmentation symptoms",
              'a 4-row markdown table plus a 3-bullet summary', y=5.15)
footer(s, T)
notes(s, "PRACTICE BOX (on-screen only — never read it, never paraphrase it, never point at it). It replaces the narrated handoff: this video ends on the recap and the Sources slide.\n\nVO: All four signs come from one root cause. There is no shared plan. No one has written down what your government's digital systems are, who owns them, and how they should fit together. That shared plan is what a national Enterprise Architecture provides. The rest of this knowledge product shows you how to commission one, what it will deliver, and what you will need from your minister to make it work.\n\nThis is the most important slide of video 1.1 — hold it a beat longer.")
...
footer(s, T)
notes(s, "The bundle folds this comparison onto the video's opening slide; in the deck it keeps its own slide, because the section slide is the standalone video's title card.\n\nVO: An EA is not software. It is not a vendor product. It is not a tool you buy. It is the agreed picture.\n\nVO: Why does it matter that everyone has the same picture? Because every important conversation in your ministry breaks down on this point.\n\nThis slide is the Strategist's first line of defence against vendor pitches — when a vendor says 'our product gives you an EA', this is the slide that says no, it does not.")
...
footer(s, T)
notes(s, "VO: Your minister uses the picture to brief cabinet. They cannot describe what the country's digital spend is buying if there is no agreed picture of what your government's systems are. The donor uses the picture before they fund the next programme. They want to see how their investment fits with the others. The vendor uses the picture when they propose a system. They must match what is already there. And you, the middle manager, use the picture to keep all three aligned — so the donor funds what the country needs, the vendor builds what fits, and the minister tells a coherent story.")
...
big_slide(prs, 'An EA is the agreed picture of your government — and the tool that lets you lead the conversation about what comes next.',
          T,
          "PRACTICE BOX (on-screen only — never read it, never paraphrase it, never point at it). It replaces the narrated handoff: this video ends on the recap and the Sources slide.\n\nVO: That is what an Enterprise Architecture is. Not software. Not a tool. The agreed picture of your government — and the tool that lets you, instead of the vendor or the donor, lead the conversation about what comes next.\n\nScreenshot-ready summary; Strategists reuse this line in their own briefings.",
          practice=("Draft a one-slide 'what is EA' explainer for ministers",
                    "a one-slide structure ready to paste into the country's slide template"))
...
big_slide(prs, 'Re-use, sustainability, complexity reduction — none of these come from projects. All of them come from planning.',
          ...
          practice=('Build the whole-of-government business case for building-block re-use',
                    'a per-programme cost table plus a country-level 5-year saving estimate'))
...
big_slide(prs, 'In the era of digitising paper, an EA was useful. In the era of redesigning how the ministry works, an EA is necessary.',
          T,
          "PRACTICE BOX (on-screen only — never read it, never paraphrase it, never point at it). It replaces the narrated handoff: this video ends on the recap and the Sources slide.\n\nVO: In the era of digitising paper, an EA was useful. In the era of redesigning how the ministry works, an EA is necessary — because without it, business and IT cannot have the conversation the redesign requires.",
          practice=('Translate an operating-model question into a business-IT joint agenda',
...
footer(s, T)
notes(s, "Rows reveal one at a time. This is the substantive payload of video 1.5.\n\nVO: Five things PAERA gives your team on day one. Five things you do not pay to invent.\n\nVO: First, a way to organise ministries and authorities. Every government body in your country fits one of three types — a policy unit, a regulatory agency, a service-delivery authority — with a small set of supporting elements like state registries. Your architects do not spend three months arguing about which type each ministry is. The classification is published.\n\nVO: Second, plain words for the basic terms. Capability. Service. Application. Data domain. Each one defined, in plain language. Day one, your architects are using the vocabulary, not designing it.\n\nVO: Third, a library of common building blocks. Identity. Payments. Information sharing. Registries. Each one is a published specification with working examples. When your team needs to design a national identity system, they reach for the pattern. They do not invent it from scratch.\n\nVO: Fourth, a set of architectural principles. Ten of them — from rule of law, through security and privacy, to user-centred design. Already debated across many countries. You extend them for your country's context. You do not draft them.\n\nVO: Fifth, a way to run the work. The lifecycle — Discover, Assess, Adapt, Plan, Execute and Govern — already exists, with roles, decisions and sign-off points defined per phase. We walk it in the next video.")
...
big_slide(prs, 'PAERA is not a vendor choice. It is the choice to start with the work other countries have already done.',
          T,
          "PRACTICE BOX (on-screen only — never read it, never paraphrase it, never point at it). It replaces the narrated handoff: this video ends on the recap and the Sources slide.\n\nVO: PAERA is not a vendor choice. It is the choice to start with the work other countries have already done — so your team spends their time on what is specific to your country.",
          sub='So your team spends its time on what is specific to your country.',
          practice=("Map your country's existing initiatives against PAERA's five foundations",
...
set_text(tb.text_frame, [[('Screenshot this page for your EA Board room wall.', 13, False, GREY, True)]])
footer(s, T)
notes(s, "VO: Five phases, four sign-offs. The first four take about six months; the fifth is ongoing — your country's permanent way of working.")

# --- the five phases on one cumulative-reveal slide (v0.1 gave each phase its own slide)
s = add_slide(prs, LAYOUT_WHITE)
title(s, 'Each phase: one question, one deliverable, one sign-off')
rows_slide(s, [
    ('Discover · 3–4 weeks',
     'What exists today? → a Discovery brief, no recommendations yet. Sign-off: the picture is accurate.'),
    ('Assess · 6–8 weeks',
     'What is the gap? → the current state in the four parts of an EA, plus maturity scorecards. Sign-off: the gap analysis reflects ground truth.'),
    ('Adapt · 4–6 weeks',
     'What fits our country? → a localised framework with sourcing decisions. Sign-off: the framework and the build / buy / share approach.'),
    ('Plan · 6–8 weeks',
     'How do we get there? → a roadmap and investment plan, the deliverable your minister takes to cabinet. Sign-off: roadmap approved, budget committed.'),
    ('Execute & Govern · ongoing',
     'How do we sustain this? → a living, governed EA, repository plus practice. Cadence: quarterly Board reviews.'),
], top=1.5, bottom=6.9, head_size=18, sub_size=15)
footer(s, T)
notes(s, "Rows reveal one at a time, cumulative — the whole lifecycle stays visible while each phase is narrated. In v0.1 of the deck each phase had its own slide; the v0.3 script collapses them onto this one page.\n\nVO: Phase one, Discover. What exists today? Your architects map the current landscape — the strategies in force, the systems, the sector plans, the legal framework — and produce a Discovery brief with no recommendations yet. Sign-off: the picture is accurate enough to build on. Three to four weeks.\n\nVO: Phase two, Assess. What is the gap? Architects compare what they found against PAERA-anchored standards, write the current state in the four parts of an EA, score maturity, and write the gap analysis. Sign-off: it names the right problems in the right order. Six to eight weeks.\n\nVO: Phase three, Adapt. What fits our country? PAERA is a starting point, not a constraint, so architects shape it with sector CIOs and your EA Board — your principles, your sector priorities, and for each building block a build, buy, share or sandbox call. Four to six weeks.\n\nVO: Phase four, Plan. How do we get there? Architects describe the target state and sequence the work into a roadmap with investment estimates. Sign-off: roadmap approved, budget committed. This is the deliverable your minister takes to cabinet. Six to eight weeks.\n\nVO: Phase five, Execute and Govern. How do we sustain this? The approved roadmap becomes a project pipeline, and a small permanent team of two to four senior architects turns the EA from a one-time delivery into a living repository. The Board reviews new projects against the architecture. Quarterly, indefinitely.")
s = add_slide(prs, LAYOUT_WHITE)
...
tb = box(s, 0.9, 4.65, 11.5, 1.15)
set_text(tb.text_frame, [
    [('Between sign-offs, the architects work.', 18, False, INK, False)],
    [('The minister’s job in between is to remove obstacles — the political ones, mostly. The technical ones are why the architects were hired.', 16, False, GREY, False)],
], space_after=Pt(6))
p = panel(s, 0.9, 5.95, 11.5, 0.75, LIGHT)
panel_text(s, 0.9, 5.95, 11.5, 0.75, [
    [('Discover before you measure. Measure before you adapt. Adapt before you plan. Plan before you execute. Govern always.', 15, True, ITU_BLUE_DARK, False)],
], m=0.25)
footer(s, T)
notes(s, "The dependency rule had its own slide in v0.2; the v0.3 script folds it into the sign-off rhythm.\n\nVO: Notice the rhythm. Four sign-offs in six months. Your minister does not review every diagram; they review at four moments, each tied to a defined deliverable. Between sign-offs the architects work, and the minister's job is to remove the political obstacles. And the phases depend on each other, in order: discover before you measure, measure before you adapt, adapt before you plan, plan before you execute, govern always.")

s = add_slide(prs, LAYOUT_WHITE)
title(s, 'Week 1 to week 26 — then forever')
hline(s, 0.9, 3.05, 11.5, MIDGREY, weight_pt=2.5)
miles = [
...
for lab, desc, x in miles:
    c = s.shapes.add_shape(MSO_SHAPE.OVAL, Inches(x), Inches(2.87), Inches(0.4), Inches(0.4))
    solid(c, ITU_BLUE)
    tb = box(s, x - 1.0, 1.95, 2.5, 0.6)
    set_text(tb.text_frame, [[(lab, 18, True, INK, False)]], align=PP_ALIGN.CENTER)
    tb = box(s, x - 1.25, 3.55, 3.0, 1.0)
    set_text(tb.text_frame, [[(desc, 14, False, GREY, False)]], align=PP_ALIGN.CENTER)
tb = box(s, 0.9, 4.4, 11.5, 0.65)
set_text(tb.text_frame, [
...
s = add_slide(prs, LAYOUT_WHITE)
title(s, 'Four very different governments have committed to this')
rows_slide(s, [
    ('Rwanda — small country, strong centre',
     'Irembo as the one citizen-services platform; a national ID linked across services.'),
    ('Kenya — one-stop centres and a unifying identity programme',
     'Huduma Centres and Huduma Namba. Results mixed and openly debated — which is what makes the obstacles useful to you.'),
    ('South Africa — federal',
     'Coordinated through SITA and shared standards that agencies adopt, not a single architecture imposed from the top.'),
    ('Estonia — mature',
     'Distributed registries on X-Road, the once-only principle, almost every service online. A polestar, not a template.'),
], numbered=False, head_size=18, sub_size=15)
footer(s, T)
notes(s, "Honesty about Kenya's contested experience is a feature of this example, not a defect.\n\nVO: One more thing to take into the meeting. Four very different governments have already made these commitments. Rwanda, small with a strong centre, one citizen-services platform and a national identity linked across services. Kenya, with one-stop centres and an identity programme whose results are mixed and openly debated. South Africa, federal, coordinating through a central agency and shared standards rather than imposing them. And Estonia, mature, with distributed registries and the once-only principle.")

s = add_slide(prs, LAYOUT_WHITE)
title(s, 'The same four elements show in all four')
rows_slide(s, [
    ('A small central team with real authority',
     'Ask 1 — the small permanent EA team. Rwanda coordinates centrally; South Africa does it through SITA.'),
    ('Binding governance, not advisory',
     'Ask 2 — an EA Board whose decisions hold. Where authority cannot be imposed, it is agreed and then binding.'),
    ('A published framework other agencies adopt',
     'Ask 3 — the sustained envelope. A framework stays adopted only while someone is funded to maintain it.'),
    ('A multi-year horizon, results visible in months',
     'Ask 4 — the promise. Estonia took two decades; Rwanda showed results inside months. Both protected the team.'),
], numbered=False, head_size=19, sub_size=15, bottom=6.35)
tb = box(s, 0.72, 6.4, 11.9, 0.5)
set_text(tb.text_frame, [[('Different sizes, different constitutions — the same four elements, which are the four asks in four real governments.', 14.5, False, GREY, False)]])
footer(s, T)
notes(s, "VO: Four contexts, one pattern. A small central team with real authority. A published framework other agencies adopt. Binding governance rather than advisory. And a multi-year horizon with results visible inside months. Different sizes, different constitutions, the same four elements — which are the four asks, in four real governments.")

big_slide(prs, 'A team. A Board. A budget envelope. A promise. Put them on one page — and ask for all four together.',
          T,
          "PRACTICE BOX (on-screen only — never read it, never paraphrase it, never point at it). It replaces the narrated handoff: this video ends on the recap and the Sources slide.\n\nVO: Four asks. A team. A Board. A budget envelope. A promise. Put them on a single page and ask for all four together — not in pieces. With all four committed, you have what you need.",
          practice=('Draft a Terms of Reference for your EA Governance Board',
                    'a structured ToR document'))

sources_slide(prs, T, [
    'Rwanda — Irembo (irembo.gov.rw); Kenya — Huduma Kenya (huduma.go.ke)',
    'South Africa — SITA (sita.co.za); Estonia — e-Estonia.com, RIA (ria.ee)',
    'PAERA v1.0 — §4.2.1 Management; §4.2.2 Architecture',
    'PAERA v1.0 — §5.4 Organisational Assessment & Roadmap',
])
