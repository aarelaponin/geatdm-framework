# NotebookLM setup for KP1 · M5 · Video 5.5

Deck `KP1_M5_5.5_Deck_v0.2.pptx` · brief `KP1_M5_5.5_AudioBrief_v0.2.md`
...
1. Use the notebook reserved for video 5.5 (`notebooks.json`), or create one used only for it.
2. **Reset its sources.** Remove every earlier brief — an older one left in the notebook gets
   blended into the take.
3. Add `KP1_M5_5.5_AudioBrief_v0.2.md` as the **only** source.
...
```
Follow the source titled "AUDIO BRIEF — KP1 Module 5 Video 5.5" exactly. It is the sole
...
```
Follow the source "AUDIO BRIEF — KP1 Module 5 Video 5.5" exactly, working through its
...
python3 …/kp-audio-brief/scripts/srt_drift_check.py \
  videos/module_5/en/audio/KP1_M5_5.5_Audio_v0.X.srt --target 300 --tolerance 45
```
