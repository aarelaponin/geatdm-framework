# NotebookLM setup for KP1 · M2 · Video 2.7

Deck `KP1_M2_2.7_Deck_v0.2.pptx` · brief `KP1_M2_2.7_AudioBrief_v0.2.md`
...
1. Use the notebook reserved for video 2.7 (`notebooks.json`), or create one used only for it.
2. **Reset its sources.** Remove every earlier brief — an older one left in the notebook gets
   blended into the take.
3. Add `KP1_M2_2.7_AudioBrief_v0.2.md` as the **only** source.
...
```
Follow the source titled "AUDIO BRIEF — KP1 Module 2 Video 2.7" exactly. It is the sole
...
```
Follow the source "AUDIO BRIEF — KP1 Module 2 Video 2.7" exactly, working through its
...
python3 …/kp-audio-brief/scripts/srt_drift_check.py \
  videos/module_2/en/audio/KP1_M2_2.7_Audio_v0.X.srt --target 300 --tolerance 45
```
