...
# Every Security Server's /etc/xroad/conf.d/local.ini, generated so the one
# tunable in it -- the proxy's authorization-cache period -- is declared in
# deployment.yaml beside bind and the digest pins, not buried in a
# hand-written ini nobody reads before copying. This was
# xroad-demo-local.ini; its comment block (below, verbatim) is the reason the
# file exists at all and travels with it.
#
# [proxy-ui-api] wsdl-validator-command is the sidecar image's own default,
# repeated here so this file can fully REPLACE the packaged one -- the image's
# entrypoint only writes that one on first boot and never touches it again
# (/root/_entrypoint_common.sh's `cp -a -n ... backup/local.ini`). Drop the
# section and the demo loses the validator, not just the comment.
LOCAL_INI = """\
# GENERATED by hurl/generate.py from deployment.yaml -- do not hand-edit.
...
# CPU for faster-to-reflect ACL changes). The value itself is
# deployment.yaml's xroad.server_conf_cache_period.
[proxy-ui-api]
wsdl-validator-command = /usr/share/xroad/wsdlvalidator/bin/wsdlvalidator_wrapper.sh

[proxy]
server-conf-cache-period = @period@
"""

# The path docker-compose.yml and member_service_block() both mount. Relative
# to the pack root (Compose resolves relative bind mounts against the project
# directory, which is where docker-compose.yml lives), NOT to this file.
...
def member_service_block(key: str, dns: str, ui: int, rest: int) -> str:
    ...
    return (
        ...
        f"      - {LOCAL_INI_MOUNT}:/etc/xroad/conf.d/local.ini\n"
        f"    healthcheck:\n"
...
def main() -> None:
    ...
    # -- local.ini ---------------------------------------------------------
    period = (deployment.get("xroad") or {}).get("server_conf_cache_period")
    if not isinstance(period, int):
        raise SystemExit(
            "generate.py: deployment.yaml needs xroad.server_conf_cache_period "
            f"as an integer number of seconds (found: {period!r}). It is the "
            "proxy authorization-cache period every Security Server boots with "
            "-- X-Road 7.7.0's own default is 60; this pack ships 5 so an ACL "
            "change is filmable."
        )
    (HURL_DIR / "local.ini").write_text(sub(LOCAL_INI, period=period))
    print("  wrote hurl/local.ini")
