...
That does both corpora, verifies them with `tests/test_golden.py`, and
prints the diffstat without committing anything. It was previously a manual
sequence — `--out` into a temp directory, `rm -rf` the corpus, `cp -r` over
it — and the `tests/golden/hosted-fixture/generated/` half was worse: it has
to run from a *copy* of `generate.py` against
`tests/golden/hosted-fixture/member-configs/` rather than the real
`configs/`, so refreshing it by hand meant letting `tests/test_golden.py`'s
`_generate_hosted_fixture()` build the temp copy and grabbing
`hosted-fixture-out/` before the temp directory was cleaned up (`pytest
--pdb`, or copying the function body). `regen-golden.sh` calls that same
function rather than reimplementing it, so the corpus is regenerated by
exactly the code that verifies it.

The fixture member-configs are a copy of the real `configs/` and can drift
from them. Two tests in `tests/test_golden.py` draw the line: the fields
`generate.py` reads must match the real configs exactly, and only the
documented decoration fields (`sla:`, `member_requirements:` — read by
`scripts/render_onboarding.py`, never by `generate.py`) may be absent.
