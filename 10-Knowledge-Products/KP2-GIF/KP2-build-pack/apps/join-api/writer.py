...
_ADMISSION_NOT_IMPLEMENTED = "**named absence** -- not implemented in this demo, see `docs/production-delta.md`"
...
def render_gates_table(has_services: bool, *, admitted: bool = False) -> str:
    ...
    sla_status = (
        ...
        if has_services
            ...
            "**out of scope** -- no services published, and a consumer-only "
            "member has none (TK-IO-09 is written for providers; the "
            "onboarding path's own §8 open question 5)"
        )
    )
    catalogue_status = (
