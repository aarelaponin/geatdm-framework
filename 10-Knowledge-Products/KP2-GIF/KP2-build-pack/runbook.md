...
- **Join via the API (automated):** `scripts/join.sh {up|down|status}`
  ...
  — `code`, `subsystem`, and `security_server.dns_name` must follow the
  identifier, member-code, and host-naming conventions `docs/conventions.md`
  publishes, or validation rejects the request before it reaches an operator
