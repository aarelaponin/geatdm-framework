...
exists — four keys, read by `validate.py`'s fourteen per-request checks
(eleven from the original design, plus `lawful_basis`, `sla_required` and
`spec_url_origin`, added later) and by `hurl/generate.py`'s
