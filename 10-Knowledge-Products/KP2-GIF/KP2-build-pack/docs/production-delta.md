...
| **Partly closed, and the halves are not equal.** The demo's mock backends now serve TLS on `:8443` beside plain `:8000`, with a certificate the Test CA issued for the container's own Compose name — no new issuance mechanism was needed, the Test CA's existing `POST /testca/sign` with `type=auth` already carries `extendedKeyUsage = clientAuth, serverAuth` and copies the CSR's `subjectAltName` through, so `apps/mock-registry/entrypoint.sh` asks for its own certificate at boot exactly the way a Security Server asks for its AUTH certificate. The fixtures' `spec_url`s moved with it (`configs/member-plr/plr.yaml`, `configs/member-pnia/pnia.yaml` — `https://app-{plr,pnia}:8443/spec.yaml`), and `deployment.yaml`'s new `join_workflow.require_https_spec_url` (default `false` for the transition, `true` on the droplet, switched by `posture:` -- `posture: production` implies it) makes `apps/join-api/validate.py`'s `https_spec_url` check REJECT a plain-`http` `spec_url` before check 9 fetches it. **Verified means verified on exactly one hop.** `apps/spec-fetcher/app.py` builds an `ssl.create_default_context()` and ADDS the deployment's CA bundle to it (`SPEC_FETCHER_CA_BUNDLE`, the `ca-certs` volume — public certificates only, never `ca-data`, which also holds the CA private key); never `verify=False`. So the fetch join-api makes at validate time is genuinely authenticated. **The Security Server's OWN fetch of the same `spec_url` is not** — measured against 7.7.0 by serving a self-signed certificate from an unrelated container on `linkup` and adding it as a service description: `POST /service-descriptions` returned **201** and parsed the document. X-Road does not verify the backend certificate on a service-description fetch, so that hop is encrypted and unauthenticated, and no switch in this pack changes it. Also still open: the service data hop itself (`servers[].url` inside each spec) is still plain `http://app-*:8000/v1`. X-Road CAN verify it (`ssl_auth: true` on the service plus the backend certificate uploaded to the client's `tls-certificates` — both API calls confirmed working), but it PINS the uploaded certificate, and this pack's mock certificates are issued per container boot while `scripts/seed.sh` restarts the mocks — a pinned certificate would go stale on a routine demo operation | HTTPS to information systems on **every** hop, certificates verified on every hop: a real CA, certificates that outlive a container restart, `ssl_auth: true` on each service with the backend's certificate registered, and a Security Server whose service-description fetch is itself validated (an X-Road-side gap, not a configuration this pack can set) |
| **Partly closed — transport, not client authentication.** `configs/member-pnea/pnea.yaml`'s `client.connection_type` is now `HTTPS_NO_AUTH`, and X-Road enforces it: the plaintext client proxy is refused for that client outright — measured, `{"type":"Server.ClientProxy.SslAuthenticationFailed","message":"Client (SUBSYSTEM:PROGRESSA/GOV/PNEA/EXAMS) specifies HTTPS NO AUTH but client made plaintext connection"}`, HTTP 500 — while the same call on `:8443` returns the record. The hop is also *verifiable*, which took one more step: out of the box the sidecar's internal TLS certificate is self-signed with the CONTAINER ID as its CN and no `subjectAltName`, so nothing could verify it without pinning. A new Hurl step (`ss.internal_tls_cert`, rendered only for a server whose own client is not plain `HTTP`) asks the server for a CSR (`POST /system/certificate/csr`, `CN=ss-pnea`), has the Test CA sign it, and imports it (`POST /system/certificate/import`) — so every caller verifies against the CA bundle instead. Every consumer caller moved to it with verification ON, never `-k` and never `verify=False`: `scripts/lib-stack.sh`'s `rest_base()` (`--cacert` + `--resolve`, so the name in the certificate is the name verified while the connection stays on loopback), `scripts/acceptance.sh`, `apps/console/xroad.py` (a separate `EXCHANGE_CLIENT`, never sharing a trust decision with the admin API's own client), and `apps/join-api/job.py`'s `join.r1_verify`. **The `:4000` admin plane is now TOFU-pinned, not merely unverified** (security-review-remediation-plan.md Phase C, M1): `hurl/run-linkup.sh` captures each server's own certificate at deploy time into `out/xroad-admin-certs/<host>.pem`, and `apps/console/xroad.py`'s `admin_client()`, `scripts/member.sh`'s `refresh`, and `scripts/lib-stack.sh`'s `api_key()`/`api()` (`curl --pinnedpubkey`, since curl has no CLI-only "verify chain, skip hostname" flag) all pin against the captured leaf instead of running unverified — `docs/decisions/xroad-admin-tofu-pinning.md` records why hostname verification stays off and what TOFU does and does not close. `apps/join-api/job.py`'s Hurl `--insecure` and its `verify=False` reachability probe are the one path deliberately NOT re-plumbed to pin (decision 3 in the plan); `posture: production` instead refuses to run either without `join_workflow.hurl_insecure: true` and `hurl_insecure` in `join_workflow.acknowledge_permissive`. **Deliberately NOT closed: client authentication.** `HTTPS_NO_AUTH` authenticates the Security Server to the information system, not the information system to the Security Server. Full `HTTPS` additionally requires a client TLS certificate issued to, distributed to and registered for every consumer caller — the host scripts, the console container and the join-api container all make this call — which is a distinct security property from transport encryption and is not implemented here. Read this row as "the consumer hop is off plain HTTP and its server is authenticated", never as "the consumer connection is mutually authenticated" | `HTTPS` with a client TLS certificate: the information system authenticated to the Security Server as well, certificates from a real CA, issued and rotated per consumer rather than shared |
...
| The shared `KP2_JOIN_APPLICANT_TOKEN` can now be disabled outright — setting it to the literal sentinel `disabled` (not empty: `docker-compose.yml`'s `:-` default already passes an absent value through, so absence would silently mean "disabled" the moment the line was forgotten) makes `require_applicant` skip the shared-token comparison entirely, so every applicant call must arrive on an issued per-agency credential (`POST /tokens`, hash-only storage, revocation, `submitted_by` on the record) — the console is unaffected, since it only ever holds the *operator* token server-side (`docker-compose.yml`'s own comment on that service), never the applicant one; `KP2_JOIN_OPERATOR_TOKEN` cannot be disabled the same way, `join-api` refuses to start if it is. Bearer tokens either way, expiry still optional and off by default (`expires_in_days`/`expires_at`, `app.py`/`store.py`), and rotation is still absent. Per-request ownership, recorded since the previous state of this row, is now enforceable too: `deployment.yaml`'s `join_workflow.enforce_ownership` (default `false`, docker-local, switched by `posture:` -- `posture: production` implies `true`) makes `GET /requests/{id}` **404** (not 403 — no existence oracle) for anyone but the operator, the owning `applicant:<name>` credential, or — only while the shared token is still enabled — the shared applicant reading a `submitted_by: null` record. Ownership enforcement is only meaningful once the shared token is also disabled; with it still enabled it protects per-agency records from *each other*, nothing more | Met for the droplet target: `KP2_JOIN_APPLICANT_TOKEN=disabled` plus `join_workflow.enforce_ownership: true` together are the first configuration under which this pack runs end to end without ever using a shared credential. mTLS — the delta's original ask — is still not built, expiry is still optional rather than mandatory, and rotation is still absent |
...
| `backend.auth: none` is still what every mock in this pack actually accepts, demo-only posture (`apps/join-api/schema.py`'s `BackendAuth`, exercised by the PTSB fixture) — but it is no longer unconditionally admitted. `configs/x-road-bus/join-policy.yaml`'s fifth key, `join.allowed_backend_auth` (demo default: all three values, so the PTSB fixture and every mock keep passing), is now enforced at request time (`apps/join-api/validate.py`'s `allowed_backend_auth` check, naming the policy file in its rejection message) and its shape at generate time (`hurl/generate.py`'s `check_join_policy()`) | Met for the droplet target: its policy lists `[network_allowlist, proxy_injected]` only (`runbook.md`'s production-posture notes), so a join declaring `backend.auth: none` is rejected outright, not merely discouraged in prose — the consumer must never hold the provider's own API credential |
| A joined member's service description is never automatically refreshed — X-Road reloads only on explicit refresh, so a real third-party backend (a Joget app someone edited in a browser) drifts silently from what the federation publishes. `scripts/member.sh drift <key>` detects it and `scripts/member.sh refresh <key>` remedies it, refusing when the spec as served now exceeds `join.allowed_methods` and recording the act on the join record so `drift` can distinguish drift since join from drift since the last refresh | Both halves exist as *commands*; neither is the *process*. Production needs the organisational review around them — who is notified that a member's contract moved, who decides whether the new contract is still the one that was admitted, and on what cadence anyone looks. `refresh` re-applies one policy (`allowed_methods`); a changed field set, lawful basis or SLA passes it untouched and needs a human |
...
| **Live-but-uncommitted window, forward direction — now closable:** `writer.apply_real()` writes `configs/member-<key>/` and the `manifest.yaml` entry, and the job then makes the member live on the running federation, all before anyone runs `git commit`. `deployment.yaml`'s `join_workflow.commit_gate` (default `advisory`, docker-local, switched by `posture:` -- `posture: production` implies `required`) is the switch: `advisory` is the old behaviour unchanged — `apply_real()` still refuses to *start* a new job while `configs/`/`manifest.yaml` are already dirty, and the console's join tab still surfaces the live flag (`apps/join-api/app.py`'s `_live_uncommitted`), but nothing stops a member going `ACTIVE` uncommitted. `required` (the droplet target) closes the window for real: approval plans an extra `config.commit` step, and the record `BLOCKED`s there — read-only, `git status` scoped to this join's own paths, never `git commit` itself, since the committer is the operator on the host — until `configs/member-<key>/`, `manifest.yaml`, `onboarding/<key>/` **and `onboarding/catalogue.yaml`** are committed and the operator resumes. All four, deliberately: they are exactly `writer._written_paths()`'s own four paths, everything a join actually writes — an earlier version of this gate checked only three and let a member go `ACTIVE` with the catalogue uncommitted (caught by a live re-run of this exact proof, once the first run's own evidence was checked rather than assumed clean). **The reverse direction is deliberately not gated.** `member.sh remove` (called by un-join, at the end of the reversal walk) deletes `configs/member-<key>/` before anyone commits the deletion, the mirror-image window — but blocking a *removal* on a commit would invert the risk this phase closes going forward: a member no longer on the federation but still described in git is the safe direction to drift in, a request an operator cannot finish retiring is not. The `RETIRED` record instead carries `commit_pending: true`, evidence rather than a gate, set once and cleared by nothing. **`docs/path-conformance.yaml` deliberately carries no clause for this row**: it was checked against the member-onboarding-path gate register (G0-G6, S3/S4/S6/S6a/S7, GX, P0/P1) and none of those ids name git-commit discipline for this pack's own config-as-code — `G6.1`/`S7.7` ("go-live") are production handover and first-transaction *monitoring*, a different gap; a config being version-controlled before it goes live is this pack's own engineering safeguard, not a step the onboarding path defines, so stretching an existing id to cover it would misrepresent what that id means | Met under `required`: a production join workflow should not have a window where the running system and its version-controlled description of itself can disagree at all, and now one doesn't, on the forward path. The reverse asymmetry above is a considered trade, not an oversight — restated so a later reviewer does not "fix" it into a worse failure mode |
| An in-process token bucket now limits `POST /requests` and `POST /requests/{id}/resume` to 30/minute per bearer token (`apps/join-api/app.py`), and a submission is refused once `SELECT COUNT(*) FROM requests` against the join store (`apps/join-api/store.py`) reaches 200 — a real count against a real table, not a directory listing. Reads are unlimited. Every 429 refusal is now recorded as a `request_events` row (actor = the token's digest prefix, timestamp) — a first, queryable answer, though nothing watches it yet. The rate-limit buckets themselves are still per-process, in-memory, and cleared by a restart. **Task 5 (Phase E) adds two surfaces, not a watcher:** join-api now emits structured JSON-lines logs to stdout (stdlib `logging`, `apps/join-api/join_logging.py`) — a request-id per HTTP call (contextvar, also returned as `X-Request-Id` and stamped into `request_events.detail` so a log line and its audit-table row join on the same value), the join's own record id threaded through every log line for its whole lifecycle (submit, approve, every job/unjoin step, the terminal ACTIVE/FAILED/BLOCKED/RETIRED), auth failures (token digest prefix only), 429s and job step start/end with duration — and every record passes through a scrub filter built on `job.scrub(..., _SINK_SECRETS)` — `JOB_SECRETS` (the three Hurl credentials) plus the operator/applicant bearer tokens and any Postgres DSN password — proven with an actual secret value in `tests/test_app_health.py`, not reasoned about. Persisted and returned subprocess output (the `REJECTED`/`FAILED` records `hurl/generate.py`'s whole-`.env` stderr can reach, and the applicant's own response body) now scrub against this identical `_SINK_SECRETS` set too (0.1/M3) — not the narrower `JOB_SECRETS` those six `job.scrub` call sites used to pass, which the log pipeline's own wider set never covered. `apps/console/app.py`'s own two log calls now share the identical JSON formatter (`apps/console/console_logging.py`) for consistency across both services. `GET /metrics` (operator-token gated, the same dependency every other operator route uses) is a hand-rolled Prometheus-text surface — no `prometheus_client` — covering requests by state, 429 count, job steps completed/failed, job duration (a summary), and store backend + quota usage (`store.count_requests`/the new `store.count_requests_by_state`, not a second counting mechanism) | Production needs a distributed quota — still per-process here (`_BUCKET_LOCK`, unchanged by the Postgres work below), so replicas multiply the rate limit and a restart clears the buckets regardless of which datastore backs the join store — and abuse monitoring/alerting on the now-recorded refusals: the droplet target's `runbook.md` now documents two starting queries against those records (refusals per token per hour; requests by state) **plus a third option, scrape `GET /metrics`** — still a first look, not a monitoring or alerting system. The "real datastore behind the records" this row used to also ask for now exists for the droplet target (see the row below); docker-local's default SQLite file, and the 200-record ceiling standing in for a real quota store there, are unchanged. **The structured logs and `/metrics` are additive surfaces only** — this task builds what a collector would read, not a collector: nothing scrapes `/metrics` and nothing ships the JSON logs anywhere by default, so "nothing watches it yet" stays true of this row until an operator actually points Prometheus (or a log shipper) at what now exists to be pointed at |
| **Phase E (M4) closes the rest of the request-path-limits/pre-auth-cost row.** `schema.py`'s `JoinPayload` fields now carry `Field(max_length=...)` bounds (`code`/`subsystem`/`security_server.code`: 64; `name`/`security_server.dns_name`/`hosted_on`: 200; `spec_url`: 2048; `subsystem_description`: 2000; `lawful_basis`/`technical_contact`/every `SLA` field: 500) and list-length bounds (`services`: 50; `access`/`requested_access`/`semantic.fields`: 200) — a `REJECTED` record persists the raw payload verbatim (`"payload": raw`), so an unbounded field used to be unbounded evidence per request, not just an unbounded parse. A request body over 256 KiB now gets `413` twice over: `apps/join-api/app.py`'s own ASGI middleware (byte-counted as it streams in, not Content-Length-trusted — `:8091` is reachable directly on loopback and through the console proxy, not only through nginx) and `infra/nginx/kp2-console.conf`'s `client_max_body_size 256k;` on the `location = /join/requests` block — the same number, each file's own comment naming the other. `require_applicant`/`require_operator` now compare the static bearer tokens (`compare_digest` against `APPLICANT_TOKEN`/`OPERATOR_TOKEN`) BEFORE opening any store connection, behind a cheap well-formedness regex that rejects garbage before either that compare or the issued-token DB fallback runs, and `store.init()` is memoised on `(OUT_DIR, kind, db_url)` instead of re-running its schema-scan/advisory-lock/grants work on every single request — a bad-token flood now costs a regex, not two Postgres connections. Every subprocess this API shells out to synchronously now carries a `timeout=` and surfaces `TimeoutExpired` as that call site's existing failure type, scrubbed identically: `writer._run_generate` (120s — `hurl/generate.py` runs on the request thread from `POST /requests`), `writer._git_status` (10s), and `job.py`'s per-step Hurl run (`HURL_STEP_TIMEOUT_S`, 300s — see `hurl/README.md`) | These bounds and timeouts stop this API's own request path from doing unbounded work per request; they are not a WAF, an API gateway's own field/body limits, or a substitute for the perimeter controls (and the still-open distributed-quota/per-process caveats the row above states) a real deployment needs regardless |
| Job context lives in one SQLite database on local disk (`out/join-store/join-store.sqlite3`, `apps/join-api/store.py`) for docker-local (the default) — WAL journal mode, transactional writes, an append-only `request_events` audit trail (triggers block `UPDATE`/`DELETE`), and the file is created `0600`. The droplet target can instead point the same store at a DigitalOcean-managed PostgreSQL cluster (`datastore.kind: postgres`, `apps/join-api/migrations/`) — TLS `verify-full`, a trusted-sources firewall that permits only the droplet (no public access), daily backups with point-in-time recovery, and the append-only guarantee enforced by `GRANT` rather than a trigger. See `runbook.md`'s "The Postgres join store" section | A durable, encrypted-in-transit-and-at-rest, access-controlled store — replication-capable at the infrastructure level (DO's standby-node upgrade path) — now exists for the one target that has one (the droplet); docker-local's default is still one SQLite file, one host, one process, unchanged, which is the right trade for a laptop demo. Still open on the droplet target too: distributed rate limiting proper (see the row above — unchanged, still per-process regardless of backend), abuse *monitoring* as a system rather than the runbook's two starting queries, replicas of `join-api` itself (`writer.apply_real`'s checkout write path still assumes exactly one process, on both backends — advisory locks on Postgres serialise concurrent processes instead of corrupting state, but two replicas are still not a supported shape), and an external secret store (`KP2_JOIN_DB_URL` still rests in `.env` on the droplet's disk, the same posture `docs/deployment-targets.md`'s "Where secrets rest" already names and recommends) |
...
| `Service.spec_url` and the `servers[].url` inside the spec it returns are no longer fetched from inside `join-api` **at request-validation time** (`POST /requests`, check 9a `spec_url_origin` + check 9 `backend_reachability`) — that fetch moved to `apps/spec-fetcher/`, its own container on its own `specs` Docker network (`internal: true`: no route to `cs`, `ca` or any `ss-*`, and no external egress either), holding no volumes and no secrets (`docker-compose.yml`). `join-api`'s `validate.py` calls it over HTTP (`SPEC_FETCHER_URL`) and fails closed — `SpecFetcherUnavailable`, naming the missing env var — rather than silently falling back to an in-process fetch from the credential-holding container if that variable is ever unset. The pre-existing allowlist (check 9a `spec_url_origin`: only `http`/`https`, only host names on `configs/x-road-bus/join-policy.yaml`'s `join.spec_url_hosts`, every IP literal and `localhost` refused regardless of that list, `follow_redirects=False` pinned) still runs in `join-api` before the delegated fetch, and `spec-fetcher` re-applies the identical logic a second time against the request's `allowed_hosts` (a request parameter, since it reads no policy file of its own) plus a 5s timeout and a 1 MiB response-size cap — defence in depth now that the network topology, not the list, is the primary control. `scripts/acceptance.sh` check 2.7.2 asserts the topology claim against the real running containers every run: `spec-fetcher` reachable from `join-api`, `cs:4000` (standing in for "any admin API") **not** reachable from `spec-fetcher`. The field-conformance check still adds no second fetch from the post-approval job path. **The narrower gap this row used to name is partly closed — the rule, not the network segregation** (security-review-remediation-plan.md Phase D, M2): `scripts/member.sh`'s `drift`/`refresh` commands (operator-invoked, post-admission drift detection against an *already-joined* member, not applicant-submitted at request time) used to fetch `spec_url` in-process — host-side, from `linkup` or from inside `join-api`, per `runbook.md`'s "Joining a member" section — with `urllib.request.urlopen`, no allowlist, no IP-literal refusal, and no `follow_redirects=False` either, weaker than validate.py's own pre-spec-fetcher fetch, not merely unmoved. `apps/join-api/origin.py` now holds the one implementation of the rule (`origin_error`, `no_redirect_opener`) reachable from both sides — `validate.py`'s `_origin_error` is a thin wrapper over it, and `member.sh`'s two embedded Python blocks (`cmd_drift`, `cmd_refresh`) import it directly, judging the same `join.spec_url_hosts` allowlist before every fetch and refusing any redirect outright rather than following it. **What that closes, and what it does not.** The allowlist-and-redirect-refusal *rule* is now identical on both paths. The network *segregation* the paragraph above describes for the request-validation fetch — no route to the admin plane, no secrets in the fetching container, a 5s timeout, a 1 MiB cap — is a separate property this phase did NOT extend to `drift`/`refresh`: that fetch still runs in-process, from `join-api` itself (the container holding `JOB_SECRETS` and a route to every Security Server's `:4000` admin API), on the `linkup` network — never delegated to `spec-fetcher`'s isolated `specs` network the way the request-validation fetch is. Moving it there is unstarted | Met for docker-local, for the request-validation fetch specifically: applicant-submitted URLs are now resolved and fetched from a network segment with **no route to the admin plane**, and the credentials are not in the fetching container. Still open, per target: docker-local's `specs` network has no egress at all, which is right when the only real spec hosts are the three mocks on that same network — a target with genuinely external spec hosts needs the fetcher on a network with egress but still no route to the admin plane, a decision `docs/deployment-targets.md`'s "Spec fetch egress" row now names and leaves open per target, not implemented here |
| **Container identity, and what the containers may write.** `join-api` parses applicant-controlled payloads and shells out to `hurl/generate.py`; it used to do that as **UID 0** on the droplet, with the **entire monorepo bind-mounted read-write** — `scripts/`, `apps/`, `infra/`, `.git` and `.env` included — while host scripts running as root `source`d `.env` and `hurl/topology.sh` out of that same tree. Sourcing a dotenv file executes it. That composed into a container-to-host-root path reachable from the public `/join/` surface (`docs/security-review-2026-08-23.md`, finding H1). Closed in four layers — **not four *independent* layers**, and the first version of this fix claimed exactly that and was wrong: (1) a dedicated unprivileged identity — `kp2`, uid/gid **10001**, created by `infra/terraform/cloud-init.yaml`, and **resolved by `scripts/lib-stack.sh` from the account itself** (`id -u kp2`, with `KP2_CONTAINER_UID` as an override) so that every caller gets it without having to remember an export. That last part is load-bearing and was got wrong first time round: the initial fix relied on `infra/ci/remote-deploy.sh` exporting the variable, and that script never starts these two containers — `infra/ci/console-publish.sh` does, in a separate SSH session that never saw the export, so on every normal deploy both came up as UID 0 and stayed there (`restart: unless-stopped`). A laptop, which has no `kp2` account, is unchanged; (2) `../../..:/repo:ro` with read-write child mounts for exactly `configs/`, `manifest.yaml`, `onboarding/`, `out/` and `hurl/` — `writer._written_paths()` plus the join store plus generate.py's own outputs, the boundary drawn by the kernel rather than by file ownership, and `.git` read-only too (`git --no-optional-locks status`, since a read-only `.git` cannot be index-refreshed); (3) **the host stops sourcing anything the container can write** — `scripts/lib-core.sh`'s `kp2_load_env()` parses `.env` line by line (`^[A-Z_][A-Z0-9_]*=`, one layer of quotes stripped, value exported verbatim, an unreadable line refused rather than skipped) and `kp2_load_topology()` reads `hurl/topology.json` as data with a `[A-Za-z0-9_.:-]` charset check on every key and value, so `generate.py` still *writes* `hurl/topology.sh` but nothing on the host executes it; (4) ownership as the backstop — the tree stays `root:root` (git's dubious-ownership check), `configs/`/`manifest.yaml`/`onboarding/`/`out/` become `kp2:kp2`, `hurl/` becomes `root:kp2` mode **3775** (setgid so new files land in the `kp2` group, **sticky** so a non-owner cannot unlink or rename the root-owned `generate.py`/`steps.py`/`templates/` beside its own output), and `.env` becomes `root:kp2` mode **640** — readable by the container, writable by nobody but root. `apps/join-api/Dockerfile`'s `safe.directory` also narrowed from `*` to `/repo` (review finding L4) | Met for the droplet target. **Read the layers in order, not as four independent guarantees.** Layer (1) is a precondition for layer (4): at UID 0 the ownership and sticky-bit backstop is worth nothing at all — `CAP_DAC_OVERRIDE` and `CAP_FOWNER` bypass permission bits and the sticky bit outright, and this pack sets no `cap_drop`, `security_opt` or `userns_mode` anywhere, so a root container has those capabilities. Layer (3) is the only one that survives a misconfiguration of the other three, and it is the only one to treat as load-bearing on its own. **Three residuals, none closed by this phase:** (a) `hurl/compose.members.yml` is written by `join-api` and later fed to `docker compose -f … up` **by root** — a compose fragment can declare `privileged: true` or a host-root volume, so the daemon interpreting attacker-influenced YAML as root is the same class of risk as the sourcing this phase closed, by a different mechanism. It was named in neither the review nor the remediation plan and is deliberately left open rather than rushed; (b) `hurl/local.ini` is another `generate.py` output, mounted **read-write** into all four X-Road sidecars (`docker-compose.yml`) — nothing reads it back on the host, so it is not a host-root path, but it is the same shape and `tests/test_mount_shape.py` records it as knowingly unchanged; (c) `hurl/` is still one directory holding both host-executed code and container-written output — the sticky bit makes that survivable, not clean. Retiring `topology.sh` (now that nothing sources it) and moving every generated file to its own directory would close (a), (b) and (c) together, and is the real fix |
...
`RETIRED` record in the join store (a read-only SQLite query, the same
discovery mechanism the sibling `2.7.r1`/`2.7.deny` rows use) — there
