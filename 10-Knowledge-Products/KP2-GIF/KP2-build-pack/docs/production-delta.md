...
| An in-process token bucket now limits `POST /requests` and `POST /requests/{id}/resume` to 30/minute per bearer token (`apps/join-api/app.py`), and a submission is refused once `SELECT COUNT(*) FROM requests` against the join store (`apps/join-api/store.py`) reaches 200 — a real count against a real table, not a directory listing. Reads are unlimited. Every 429 refusal is now recorded as a `request_events` row (actor = the token's digest prefix, timestamp) — a first, queryable answer, though nothing watches it yet. The rate-limit buckets themselves are still per-process, in-memory, and cleared by a restart | Production needs a distributed quota (still per-process here, so replicas multiply the rate limit and a restart clears the buckets), abuse monitoring/alerting on the now-recorded refusals, and a real datastore behind the records the 200-record ceiling is standing in for |
| Job context lives in one SQLite database on local disk (`out/join-store/join-store.sqlite3`, `apps/join-api/store.py`) — WAL journal mode, transactional writes, an append-only `request_events` audit trail (triggers block `UPDATE`/`DELETE`), and the file is created `0600` | Not replicated, not a real datastore *service* — one file, one host, one process; locks are still per-process (`BEGIN IMMEDIATE` plus `busy_timeout` avoid corruption between the API and `scripts/member.sh`'s direct-write fallback, not contention at scale). See `docs/plans/join-datastore-postgres-digitalocean-plan.md` for what's deferred |
