...
| --- | --- | --- |
| **Posture** | `posture: demo` (`deployment.yaml`, beside `target:`) — the three `join_workflow` switches below keep their own historical defaults | `posture: production` implies the safe value of `commit_gate`, `enforce_ownership` and `require_https_spec_url` at once — an explicit `join_workflow` key still wins, but an explicit permissive value under `posture: production` is a startup refusal unless the key is also named in `join_workflow.acknowledge_permissive` (the `network.bind`/`network.acknowledge_public_exposure` idiom, applied to a second area). Answered for the droplet target specifically (`runbook.md`'s "One statement instead of three") |
