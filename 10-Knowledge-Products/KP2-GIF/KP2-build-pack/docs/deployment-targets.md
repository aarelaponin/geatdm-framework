...
| --- | --- | --- |
| **Posture** | `posture: demo` (`deployment.yaml`, beside `target:`) — the four `join_workflow` switches below keep their own historical defaults | `posture: production` implies the safe value of `commit_gate`, `enforce_ownership`, `require_https_spec_url` and `hurl_insecure` at once — an explicit `join_workflow` key still wins, but an explicit permissive value under `posture: production` is a startup refusal unless the key is also named in `join_workflow.acknowledge_permissive` (the `network.bind`/`network.acknowledge_public_exposure` idiom, applied to a second area). `hurl_insecure`'s safe value (`false`) is not a behaviour change like the other three — it makes the whole join workflow refuse to run until acknowledged, since Hurl's admin-API calls are the one path this pack's TOFU pinning does not cover (`docs/decisions/xroad-admin-tofu-pinning.md`). Answered for the droplet target specifically (`runbook.md`'s "One statement instead of four") |
...
| **TLS verification** | **Verified against the Test CA on every data-plane hop, not disabled.** The spec fetch (`spec-fetcher`, `SPEC_FETCHER_CA_BUNDLE`), the consumer hop into the client proxy on `:8443` (`apps/console/xroad.py`, `KP2_XROAD_CA_BUNDLE`), the scripts' own r1 calls (`scripts/lib-stack.sh`'s `rest_base()`, `--cacert` + `--resolve`, never `-k`) and `scripts/member.sh`'s drift/refresh fetches all verify a real chain — the anchor is just the Test CA's rather than a public root (`docs/production-delta.md` rows 18/19). The **`:4000` admin plane** still presents each sidecar's self-signed proxy-ui certificate, but is no longer reached unconditionally unverified: `hurl/run-linkup.sh` captures each server's own certificate at deploy time and every admin caller (`xroad.py`'s `admin_client()`, `member.sh`'s `refresh`, `lib-stack.sh`'s `api_key()`/`api()`) pins against it instead — trust-on-first-use, not a real CA (`docs/decisions/xroad-admin-tofu-pinning.md`). `job.py`'s Hurl `--insecure` is the one path deliberately not re-plumbed to pin; `posture: production` refuses to run it unacknowledged instead | The same code paths, pointed at a real chain: swap the anchor (unset `SPEC_FETCHER_CA_BUNDLE`/`KP2_XROAD_CA_BUNDLE` and the stock trust store applies) — no `verify=` or `-k` to remove on the data plane, because there are none left. The `:4000` admin plane is the one that genuinely has to be decided: an accredited certificate for each Security Server's admin UI (closing even the TOFU gap), or an operator-facing terminating proxy in front of it |
