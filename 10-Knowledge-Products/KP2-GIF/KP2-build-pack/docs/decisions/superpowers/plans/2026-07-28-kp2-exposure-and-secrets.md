...
- [x] **Step 1:** rewrite `.env.example` with placeholders that cannot work:
...
- [x] **Step 2:** write `scripts/gen-secrets.sh`: generate a token PIN and an admin password from a shell-safe alphabet (`A-Za-z0-9` plus a small punctuation set excluding `$ \` ' " \\` and whitespace — see Global Constraints), at least 20 characters, from `/dev/urandom` via `LC_ALL=C tr -dc`. Write `.env` with mode `600`. Print that it wrote the file and **never print the values**.
- [x] **Step 3:** refuse to overwrite an existing `.env` without `--force`, and make `--force` print the Task 6 warning: the software token was initialised with the current PIN, so rotating it requires `scripts/teardown.sh --purge` and a redeploy.
- [x] **Step 4:** in `lib.sh`, after sourcing `.env`, refuse to proceed when `XROAD_TOKEN_PIN` or `XROAD_ADMIN_PASSWORD` is empty, still contains `CHANGEME`, or equals one of the historically published demo values (`Progressa123!`, `secret`, `Secret1234`). Name `scripts/gen-secrets.sh` in the message. **Exempt the Central Server's `xrd`/`secret`** — it is fixed in the image, `acceptance.sh` hardcodes it deliberately, and this check must not demand a change that cannot be made.
- [x] **Step 5:** in `hurl/generate.py`, change `read_env()` to **fail** when `.env` is absent instead of falling back to `.env.example`. With placeholders in the example file that fallback would now generate a `vars.env` full of `CHANGEME` and fail deep inside a Hurl run, which is the worst place to discover it.
- [x] **Step 6:** update `runbook.md` step 1 from `cp .env.example .env` to `scripts/gen-secrets.sh`, and mention it wherever `README.md` describes first run. Commit.

**Verified live (2026-07-28):** the alphabet actually shipped is narrower
than the plan's minimum exclusion list — letters, digits, and `@%+_.-` only
— because tracing through what an UNQUOTED `KEY=value` assignment does in
bash showed `; & | < > ( ) ~` are also unsafe in this exact position (end
the assignment, trigger expansion, or run a second command), not just the
five characters the plan named as a floor. Found and fixed a real bug
while testing: `LC_ALL=C tr -dc "$ALPHABET" < /dev/urandom | head -c 24`
under `set -o pipefail` fails intermittently — `head` exiting once it has
enough bytes sends `tr` SIGPIPE on its next write, and `pipefail` propagates
that as a failure, aborting the script. Reproduced on a real run (the PIN
generated fine, the password call right after it failed, identical
command). Fixed by scoping `pipefail` off for just that one substitution.
Stress-tested: 10 generated `.env` files in a row all sourced cleanly with
no parse issues. Confirmed live against the pack's OWN actual `.env`
(`Progressa123!`/`secret`, the literal committed history) that `lib.sh`
refuses it; confirmed the legitimate-generation path does not false-positive;
confirmed `generate.py` fails hard with `.env` absent. Regenerated the real
`.env`, redeployed fresh, and confirmed `scripts/acceptance.sh` and
`scripts/console.sh` both work end to end on generated, never-published
credentials — closing the loop on S2 for real, not just in the example
file.
