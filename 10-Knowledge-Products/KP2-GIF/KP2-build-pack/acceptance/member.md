...
- **Then**
  - the subsystem is `REGISTERED` on the server `hosted_on` names — its own
    server if unhosted, the server it names if hosted (the `hosted_on`
    resolution, exercised here exactly as configured);
  ...
  - a service whose `access:` list is **empty** has **no** subjects at all —
    ...
    is this pack's own example of that, for a retired member — this check
    still proves the empty case rather than assuming it, for whichever
    service happens to grant nobody today).
...
and passes on the live stack, covering the canonical three (MoEYS is retired,
and the `full`/`lite` profile split it used to be verified under both of is
also retired) and any joined member added later without a further code
change.
