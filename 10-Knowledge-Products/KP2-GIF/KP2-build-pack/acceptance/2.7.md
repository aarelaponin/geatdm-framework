...
- **Then**:
  ...
  3. **the r1 clause — the one that carries this module:** a real `r1` call,
     ...
     joined member's published service, with `X-Road-Client` set to an
     authorized consumer's identity, returns HTTP 2xx; the same call, routed
     through an *unauthorized* subsystem's own Security Server with
     `X-Road-Client` set to that subsystem's identity, is denied with an
     X-Road access-denied error (`Server.ServerProxy.AccessDenied`) — not a
     transport failure, not a 404. This mirrors `acceptance/2.6.md`'s 2.6.4/
