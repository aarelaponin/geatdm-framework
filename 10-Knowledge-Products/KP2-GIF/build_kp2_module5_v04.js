...
//   that holds for any single host; 5.3's SLA belongs to a published service; 5.2's requirements travel in the join
//   request. 5.6 is rebuilt screen-led around recorded demo evidence (KP2_M5_Theory_Demo_Video_Plan_2026-09-13): twelve
//   slides, seven of them captured from the running federation. §2 item 6 (5.7) is still pending.
...
// ---------- 5.6 ----------
// Screen-led (KP2_M5_Theory_Demo_Video_Plan_2026-09-13): slides 4–10 are demo evidence recorded from the running
// federation by the build pack's scripts/demo-capture.sh — screen frames, one clip, text captures. One observation per
// demo slide, the visible thing named before what it means; the audio brief numbers them so the hosts keep the order.
body.push(...renderSubtopic({
  ...
  words: 350,
  shape: "screen-led",
  paeraAnchor: "PAERA §5.2 Principle #5 (Once-Only); NIIS X-Road; the build-pack acceptance check",
  ...
  slideSpecRows: [
    ["1", "Title card (section slide). Title: 'Run the once-only exchange, live'.",
      "Standard ITU template. The silent cold open. No images."],
    ["2", "Call-flow diagram: the learner → PNEA → PNIA and PLR, over the bus.",
      "The drawn 'before' picture: the recording's frames map onto its boxes. Shapes and text only."],
    ["3", "Demo evidence (screen frame). The form before the call: ten blank rows. Caption strip, provenance line.",
      "Recorded from the running federation (beat C1-before). Pixels — the ITU calibration item in Section 5."],
    ["4", "Demo evidence (screen frame). The form after the call: asked 1, pre-filled 9 / 9, every row labelled with its source.",
      "Beat C2-after. Calibration item."],
    ["5", "Demo evidence (screen frame). The layer view, legal pane: what PNIA sends and what it holds but withholds.",
      "Beat C4-layers. Replaces v0.3's 'every layer' rows slide — the console says the same with live values. Calibration item."],
    ["6", "Demo evidence (screen frame). The same request from two callers: PNEA allowed, PLR AccessDenied.",
      "Beat C5-allowed-denied. Calibration item."],
    ["7", "Demo evidence (clip). Withdraw PNEA's grant: the PNIA half fails, PLR still fills; restore. Still = the denied moment.",
      "Beat C6-break-restore, ~18 s, plays once then holds its last frame. The one clip. Calibration item."],
    ["8", "Demo evidence (text capture). The assembled application, one line per field with its source.",
      "Beat C7-application. Monospaced text on the grey panel — inside the text-only rule."],
    ["9", "Demo evidence (text capture). The acceptance summary, 2.6.1–2.6.6 PASS. Caption carries the go-live line.",
      "Beat C8-acceptance. Text — inside the text-only rule."],
    ["10", "Single-sentence summary slide. One large text block (Arial Bold 28pt) and the practice box.",
      "The take-home line that crowns the module."],
...
body.push(...renderSubtopic({
  ...
  scriptBeats: [
    ...
    { cue: "Slide 5 — Title: 'Sources'. Body: the deliverables of this knowledge product — the decree (Module 2), the Governance Pack (Module 3), the standards portfolio (Module 4); EU EIF. Footer: 'Find the link in the description.'" }
  ],
  slideSpecRows: [
