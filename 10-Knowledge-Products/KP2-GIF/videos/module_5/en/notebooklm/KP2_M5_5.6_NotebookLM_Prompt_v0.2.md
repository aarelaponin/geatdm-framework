# NotebookLM setup for KP2 · M5 · Video 5.6

Deck `KP2_M5_5.6_Deck_v0.2.pptx` · brief `KP2_M5_5.6_AudioBrief_v0.2.md`
...
1. Use the notebook reserved for video 5.6 (`notebooks.json`), or create one used only for it.
2. **Reset its sources.** Remove every earlier brief — an older one left in the notebook gets
   blended into the take.
3. Add `KP2_M5_5.6_AudioBrief_v0.2.md` as the **only** source.
...
```
Follow the source titled "AUDIO BRIEF — KP2 Module 5 Video 5.6" exactly. It is the sole
...
Slides 4 to 10 are a recorded demonstration: describe them as seven numbered observations in the brief's order — "First", "Second", … — skipping, merging and reordering none, and describing nothing on screen that the brief does not quote. Inside those slides Host A asks no question; Host B opens each observation with its number.

Total runtime about 4 minutes. Never exceed 5 minutes.
...
Say "the reference architecture" in words; never say the initialism "PAERA" or any expansion of it. "building block" — never "module" or "component". Invent no figures, dates, countries or examples.
```

## Step 4 — Fallback if the box truncates

```
Follow the source "AUDIO BRIEF — KP2 Module 5 Video 5.6" exactly, working through its
...
examples. About 4 minutes, never over 5. No filler, no backchannel, no metaphors, no
...
- [ ] Runtime 3:30–4:30, and under 5:00
...
- [ ] 12 clean segment boundaries you can cut cues against

```bash
python3 …/kp-audio-brief/scripts/srt_drift_check.py \
  videos/module_5/en/audio/KP2_M5_5.6_Audio_v0.X.srt --target 240 --tolerance 45
```
