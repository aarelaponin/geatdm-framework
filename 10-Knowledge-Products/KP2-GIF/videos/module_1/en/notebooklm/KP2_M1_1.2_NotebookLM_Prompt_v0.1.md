# NotebookLM setup for KP2 · M1 · Video 1.2

Deck `KP2_M1_1.2_Deck_v0.1.pptx` · brief `KP2_M1_1.2_AudioBrief_v0.1.md`
...
3. Add `KP2_M1_1.2_AudioBrief_v0.1.md` as the **only** source.
...
```
Follow the source titled "AUDIO BRIEF — Module 1 Video 1.2" exactly. It is the sole
...
Say "the reference architecture" in words; never say the initialism "PAERA" or any expansion of it. Invent no figures, dates, countries or examples.
```

## Step 4 — Fallback if the box truncates

```
Follow the source "AUDIO BRIEF — Module 1 Video 1.2" exactly, working through its
...
- [ ] 10 clean segment boundaries you can cut cues against

```bash
python3 …/kp-audio-brief/scripts/srt_drift_check.py \
  videos/module_1/en/audio/KP2_M1_1.2_Audio_v0.X.srt --target 300 --tolerance 45
```
