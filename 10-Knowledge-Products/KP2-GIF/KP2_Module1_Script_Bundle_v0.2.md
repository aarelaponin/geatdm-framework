<!-- GENERATED from build_kp2_module1_v02.js by bundle_to_md.py — do not hand-edit; edit the build script and regenerate. -->

# KP2 Module 1 — Video Script Bundle v0.2 (ITU-aligned)
...
| --- | --- | --- | --- |
| 1.1 | Why interoperability can't be bought, only built | Procurement rules can require interoperability. Only whole-of-government planning delivers it — and once the framework exists, procurement is how you enforce it. | ~5 min |
...
> **Single message —** _Procurement rules can require interoperability. Only whole-of-government planning delivers it — and once the framework exists, procurement is how you enforce it._
...
> _Slide 6 — Title: 'You will still buy — the framework changes what the tender says'. Body, three text rows: 'The platform, the security servers, the integration work are procured — against the framework's reference architecture as the spec.' 'The framework's rules go into every tender as mandatory requirements; the buyer owns the architecture, not the vendor.' 'The contract is for a working, accepted platform — with acceptance tests, warranty and penalties — not for advice.'_
...
> _Slide 7 — Title: 'In one sentence'. Body, large text (Arial Bold 28pt): 'Procurement can require interoperability. Only a planned framework can deliver it — and once it exists, procurement is how you enforce it.'_
...
> _Slide 8 — Title: 'Sources'. Body: EU European Interoperability Framework; NIIS X-Road (niis.org); PAERA v1.0 §3.4.3; §5.2. Footer: 'Find the link in the description.'_
...
| 6 | Framework-then-procurement slide. Three text rows: what is still bought, what the tender now says, what the contract is for. | The other half of 'built, not bought' — procurement as the enforcement lever once the framework exists. Text-only. |
| 7 | Single-sentence summary slide. One large text block (Arial Bold 28pt). | The take-home line, quotable in a cabinet briefing. |
| 8 | Sources slide. Bulleted text. Footer: 'Find the link in the description.' | Lets viewers verify the framing via the YouTube description. |
...
> _Slide 7 — Title: 'The three seams'. Body, three text rows: 'Legal authorises organisational — no decree, no authority to set standards or sanction.' 'Organisational governs technical and semantic — the operating authority publishes the standards and curates the vocabulary.' 'Technical and semantic make the legal obligations enforceable — once-only cannot be enforced until the bus and the shared meaning exist.'_
...
> _Slide 8 — Title: 'In one sentence'. Body, large text (Arial Bold 28pt): 'Interoperability is four layers — technical, semantic, organisational, legal. It fails at whichever one you skip.'_
...
> _Slide 9 — Title: 'Sources'. Body: EU European Interoperability Framework (four-layer model); NIIS X-Road documentation; PAERA v1.0 §3.4.3. Footer: 'Find the link in the description.'_
...
| --- | --- |
| Persona | S (Strategist) — national interoperability authority, ministry CIO, Ministry of Justice sponsor, or development-partner lead |
| Target runtime | ~4 min (≈580 spoken words) |
...
> _Slide 4 — Title: 'Who to work first'. Body, four text rows: 'Pair a Champion consumer with a willing provider — that is your first exchange.' 'Use the first win to move Early Adopters.' 'Bring Observers along with evidence, not argument.' 'Lock the Champions in writing: a one-day inception workshop, then a signed commitment letter from each.'_
...
**Inputs and outputs:** Input: a list of agencies with their data roles and attitudes. Output: a tier map, a provider/consumer table, a recommended onboarding sequence, and a one-paragraph commitment-letter template for the Tier 1 ministries.
