...
body.push(...renderSubtopic({
  ...
  scriptBeats: [
    ...
    { cue: "Slide 3 — Title: 'Generate, then confirm against the registry'. Body, two text rows: 'The AI prompt drafts the subsystem and the access-control entries from the member's details and the access policy.' 'Every identifier is a [confirm] until checked against the live X-Road registry — a wrong member code routes nowhere, or to the wrong agency.'" },
    { text: "The AI prompt for this video generates these from the member's details and the access policy. But every identifier — the member code, the subsystem code, the certificate references — is a [confirm] until you check it against the live X-Road registry. This is the place the confirm discipline matters most in the whole framework: a wrong member code does not throw a clear error. It silently routes nowhere, or worse, to the wrong agency, which in an interoperability bus means one citizen's data going to a service that asked about another. Generate the registration; confirm every identifier against the registry before you deploy it." },
  ...
  slideSpecRows: [
    ...
    ["3", "Generate-then-confirm slide. Two text rows: the AI prompt drafts; [confirm] against the registry.",
...
body.push(...renderSubtopic({
  ...
  scriptBeats: [
    ...
    { cue: "Slide 4 — Title: 'This is the acceptance check'. Body, single text block: 'The acceptance check deploys the federation, the members, the service and the data, makes the call, and confirms once-only actually happens: the cross-server call resolves, identity and enrolment return, the learner is not asked twice. When it passes, this is not a framework explained — it is a framework that runs.'" },
    ...
    { cue: "Slide 6 — Title: 'Sources'. Body: PAERA v1.0 §5.2 Principle #5 (Once-Only); NIIS X-Road; the build-pack acceptance check. Footer: 'Find the link in the description.'" }
  ],
  slideSpecRows: [
    ...
    ["4", "Acceptance-check slide. Single text block on the acceptance check and 'explained vs running'.",
