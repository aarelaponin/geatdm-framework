<!-- GENERATED from build_kp2_module4_v02.js by bundle_to_md.py — do not hand-edit; edit the build script and regenerate. -->

# KP2 Module 4 — Video Script Bundle v0.2 (ITU-aligned)
...
Eight standalone subtopic videos. One Architect persona throughout. Total runtime approximately forty minutes. Each video has a single message and a single learning outcome, and is discoverable individually via search; the playlist provides navigation but is not required to comprehend any single video.
...
| 4.8 | Make the exchange lawful — the data-protection envelope | Letters of Interest plus a data-protection envelope make a real exchange lawful as well as technically possible. | ~5 min |
...
> _Slide 5 — Title: 'In one sentence'. Body, large text (Arial Bold 28pt): 'Adopt the four-layer reference architecture, place every component in its layer, and the gaps show you what you still have to build.'_
...
> _Slide 6 — Title: 'Sources'. Body: EU EIF / NIIS X-Road reference architecture (four functional layers); PAERA v1.0 §3.4.3. Footer: 'Find the link in the description.'_
...
| 6 | Sources slide. Footer: 'Find the link in the description.' | Lets viewers verify the reference architecture. |
...
> _Slide 3 — Title: 'What protects each crossing'. Body, three text rows: 'Between security servers: mutual TLS — both sides prove who they are.' 'The message itself is signed and logged — the X-Road message protocol.' 'The Trust-Anchor issues and can revoke the certificates.' 'The log is timestamped and itself signed — it is the evidence the legal layer relies on.'_
...
> _Slide 7 — Title: 'Sources'. Body: EU EIF / NIIS X-Road trust model (three zones); mTLS; the X-Road message protocol; RFC 3161 time-stamping. Footer: 'Find the link in the description.'_
...
| 3 | What-protects-each-crossing slide. Four text rows: mutual TLS, signed messages, the Trust-Anchor. | Derives security from zones. Technical terms glossed in plain words. |
...
**Inputs and outputs:** Input: the country (and any standards already in use). Output: a standards-portfolio table — standard, version, profile, binding date, transition period, conformance-test approach — plus the decisions that need deliberation.
...
> _Slide 5 — Title: 'Generate the semantic map, then confirm'. Body, two text rows: 'Give Claude the published vocabulary and the two agencies' field lists; it drafts the mapping.' 'Every identifier and code value is a [confirm] until checked against the real registries — an invented mapping is confident and wrong.'_
...
| 5 | Generate-then-confirm slide. Two text rows: generate, then [confirm] against registries. | Names the AI play and the anti-invention safeguard. |
| 6 | Single-sentence summary slide. One large text block (Arial Bold 28pt). | The take-home line. |
| 7 | Sources slide. Footer: 'Find the link in the description.' | Lets viewers verify the semantic standards and vocabularies. |

### AI usage tip — Generate a semantic map for an exchange

**What the prompt does:** An architect needs a semantic map for one exchange — the entities, fields, code lists, identifier and the mapping onto published vocabularies — drafted from the two agencies' field lists. This is the semantic-map play.
...
> _Slide 3 — Title: 'Generate the contract, then confirm'. Body, two text rows: 'Give Claude the service brief and the semantic map; it drafts the OpenAPI document — paths, request and response shapes, errors, security.' 'Every endpoint, field and type is a [confirm] until checked — a contract naming a field the provider does not expose fails at the first call.'_
...
| 3 | Generate-then-confirm slide. Two text rows: generate; [confirm] every field. | Names the AI play and the anti-invention safeguard. |
...
### AI usage tip — Generate an OpenAPI service contract

**What the prompt does:** An architect needs an OpenAPI 3.x contract for a service, generated from the service brief and the semantic map, ready to derive an X-Road service description. This is the service-contract play.
...
| 4 | Three-layers-meet slide. Single text block tying legal, organisational, technical together. | The synthesis across this knowledge product's layers. Text-only. |
