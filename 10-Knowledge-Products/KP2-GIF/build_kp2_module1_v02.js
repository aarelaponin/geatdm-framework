// Build KP2 Module 1 — Video Script Bundle v0.2
// v0.2 (12 Sep 2026): aligned to the 08-Interoperability source. 1.1 adds the other half of "built, not bought" — the
//   framework is what makes procurement work (its rules become mandatory tender requirements; the reference architecture
//   is the spec; the buyer owns the architecture). 1.2 adds the three seams between the layers. 1.6 adds the Tier 1
//   Commitment Letters and the inception workshop. 1.4 names the seven sections of the Strategic Foundation Document.
// Government Interoperability Framework · Topic 1 — Why interoperability, the four layers, the foundation to start
...
body.push(
  ...
  new Paragraph({ spacing: { before: 0, after: 200 },
    children: [new TextRun({ text: "Topic 1 — Why interoperability, the four layers, and the foundation to start",
      font: ARIAL, size: 24, italics: true, color: COLOR_ACCENT })] }),
  spacer(600),
  specTable([
    ["Document",            "Video script bundle for Topic 1 of KP2"],
    ["Version",             "v0.2 — aligned to ITU Knowledge Products and Video Materials Guide; aligned to the source method (12 September 2026)"],
    ["Date",                "12 September 2026 (v0.1: 27 June 2026)"],
...
body.push(
  ...
  genericTable([700, 2700, 4700, 1600], ["#", "Title", "Single message", "Runtime"], [
    ["1.1", "Why interoperability can't be bought, only built",
      "Procurement rules can require interoperability. Only whole-of-government planning delivers it — and once the framework exists, procurement is how you enforce it.", "~5 min"],
    ["1.2", "The four layers of interoperability",
      "Interoperability fails at whichever of the four layers — Technical, Semantic, Organisational, Legal — you skip. The framework makes you address all four on purpose.", "~5 min"],
    ["1.3", "The once-only promise",
      "Once-only — the state never asks a citizen for the same data twice — is the outcome interoperability is for, and the test of whether it works.", "~4 min"],
    ["1.4", "The Strategic Foundation Document",
      "Name the mandate, the scope and the principles once, in writing, before any wiring — it is what every later decision is checked against.", "~4 min"],
    ["1.5", "The Use-Case Catalogue",
      "Turn your integration map into a ranked catalogue of exchanges worth building — and start with the one that removes the most counters for citizens.", "~5 min"],
    ["1.6", "Mapping your stakeholders",
      "Sort the agencies into Champions, Early Adopters and Observers, and you know who to onboard first and who to convince.", "~4 min"],
    ["1.7", "What the world already proved",
      "Estonia's X-Road and the EU's once-only systems show the pattern; the education semantic standards show the vocabularies you will reuse rather than invent.", "~4 min"]
  ]),
  pageBreak()
);
...
body.push(...renderSubtopic({
  ...
  title: "Why interoperability can't be bought, only built",
  runtime: "~5 min",
  words: 660,
  paeraAnchor: "EIF four layers; NIIS X-Road; PAERA §3.4.3 (interoperability framing); §5.2 (Whole-of-Government, Once-Only)",
  singleMessage: "Procurement rules can require interoperability. Only whole-of-government planning delivers it — and once the framework exists, procurement is how you enforce it.",
  scriptBeats: [
    { cue: "Slide 1 — Title: 'Why interoperability can't be bought, only built'. Voice-over begins." },
    { text: "Your government has probably already required interoperability. Every new system contract says the supplier must provide open interfaces. The national digital strategy says systems must connect. And yet the citizen still gives her ID number to the health ministry on paper, and gives it again on paper when she enrols her child in school. The requirement is written down. The result is not there. The question this video answers is why — and what actually works instead." },
    { cue: "Slide 2 — Title: 'A rule can require behaviour. It cannot make it the cheapest choice.'. Body, two text rows: 'The contract says: provide open interfaces.' 'The project asks: what is the fastest way to ship on time and on budget?'" },
    ...
    { cue: "Slide 3 — Title: 'Every link built from scratch'. Body, three text rows: 'Last year: tax office connected to the business register — built from zero.' 'This year: health ministry needs the same data — built from zero again.' 'Next year: a third connection, from zero. The bus that should carry them all does not exist.'" },
    { text: "The result is point-to-point sprawl. Last year your team connected the tax office to the business register, and built that connection from scratch. This year the health ministry needs the same data, and the connection is built again, from scratch. Each link is private, undocumented, and breaks when either side changes. Nobody planned the shared road that every one of these journeys should travel on. There was no one whose job it was to plan it." },
    { cue: "Slide 4 — Title: 'The view that makes interoperability rational'. Body, four text rows: 'First agency pays to build the shared exchange.' 'Second agency does not — it connects once and reuses.' 'Third and fourth do the same.' 'Across the whole of government, the country builds the road once and everyone drives on it.'" },
    { text: "Interoperability becomes the rational choice only from one vantage point — the whole of government. From there the maths changes. The first agency pays to build a shared way to exchange data. The second agency does not build its own; it connects to the shared one. The third and fourth do the same. The country builds the road once, and every agency reuses it. But this view does not exist inside any single project. It exists only at the level of your country's whole digital portfolio — and someone has to hold it deliberately. That is what an interoperability framework is for." },
    { cue: "Slide 5 — Title: 'A framework, not a clause'. Body, four text rows: 'Legal — the mandate that makes exchange lawful and connection mandatory.' 'Organisational — who governs the bus, who joins, who is accountable.' 'Semantic — agreed meaning, so two agencies mean the same thing.' 'Technical — the shared bus and standards every member uses.'" },
    { text: "A framework is the plan, made of four parts that a contract clause cannot deliver. A legal mandate that makes data exchange lawful and connection mandatory. An organisational layer that names who owns the bus, who may join, and who is accountable. A semantic layer so that two agencies mean the same thing by the word 'learner'. And a technical layer — the shared bus and the standards every member uses. This knowledge product builds all four, in order, for a real exchange. The point here is that no procurement clause produces any of them. Only planning at the level of the whole government does." },
    { cue: "Slide 6 — Title: 'You will still buy — the framework changes what the tender says'. Body, three text rows: 'The platform, the security servers, the integration work are procured — against the framework's reference architecture as the spec.' 'The framework's rules go into every tender as mandatory requirements; the buyer owns the architecture, not the vendor.' 'The contract is for a working, accepted platform — with acceptance tests, warranty and penalties — not for advice.'" },
    { text: "None of this means you stop procuring. You will still buy — the central platform, the security servers, the integration services. What the framework changes is what the tender says. Once the framework exists, its reference architecture is the specification, its rules go into every new tender as mandatory requirements, and the country owns the architecture rather than inheriting whichever one the winning vendor brought. And the platform itself is bought as a working, accepted result — with acceptance tests, a warranty and penalties for delay — not as a consulting exercise. So the sequence is the whole lesson: plan the framework first, then let procurement enforce it. The toolkit behind this knowledge product carries that path, from reference architecture to an issuable tender." },
    { cue: "Slide 7 — Title: 'In one sentence'. Body, large text (Arial Bold 28pt): 'Procurement can require interoperability. Only a planned framework can deliver it — and once it exists, procurement is how you enforce it.'" },
    { text: "That is the case to make. Rules can require interoperability; only a framework, planned once for the whole of government, can deliver it — and once it exists, every tender carries its rules. Framework first, procurement as its lever." },
    { cue: "Slide 8 — Title: 'Sources'. Body: EU European Interoperability Framework; NIIS X-Road (niis.org); PAERA v1.0 §3.4.3; §5.2. Footer: 'Find the link in the description.'" }
  ],
  slideSpecRows: [
    ["1", "Title slide. Title: 'Why interoperability can't be bought, only built'.",
      "Standard ITU template. Title Arial Bold 28pt; subtitle (KP2 / 1.1) Arial 18pt. Background #E5F5FB. No images."],
    ["2", "The rule-vs-cheapest-choice contrast. Two text rows: what the contract says, what the project asks.",
      "Frames the local-rational behaviour without judging it — the maths, not the morality."],
    ["3", "Point-to-point sprawl. Three text rows showing the same connection rebuilt year after year.",
      "The recognition moment — the listener has lived this. Text-only, no diagram needed."],
    ["4", "Whole-of-government math. Four text rows: build once, reuse many. Concludes that the view exists only at portfolio level.",
      "The structural argument: planning enables re-use. The most important slide in the video."],
    ["5", "Framework-not-clause. Four text rows naming the four layers in plain language.",
      "Previews 1.2. Text labels only."],
    ["6", "Framework-then-procurement slide. Three text rows: what is still bought, what the tender now says, what the contract is for.",
      "The other half of 'built, not bought' — procurement as the enforcement lever once the framework exists. Text-only."],
    ["7", "Single-sentence summary slide. One large text block (Arial Bold 28pt).",
      "The take-home line, quotable in a cabinet briefing."],
    ["8", "Sources slide. Bulleted text. Footer: 'Find the link in the description.'",
      "Lets viewers verify the framing via the YouTube description."]
  ],
  aiTip: {
    title: "Diagnose where your country procures interoperability instead of planning it",
    problem: "A Strategist needs to show, concretely, that the country's current approach relies on procurement clauses that cannot deliver — and to identify where a planned framework would change the outcome. This prompt produces a defensible diagnostic the Strategist can take into a budget or cabinet discussion.",
    ...
    io: "Input: 1-3 paragraphs on the country's current cross-agency exchange approach. Output: a diagnostic table plus a 3-bullet summary.",
...
body.push(...renderSubtopic({
  ...
  title: "The four layers of interoperability",
  runtime: "~5 min",
  words: 660,
  paeraAnchor: "EIF four-layer model (Technical, Semantic, Organisational, Legal); NIIS X-Road; PAERA §3.4.3",
  singleMessage: "Interoperability fails at whichever of the four layers — Technical, Semantic, Organisational, Legal — you skip. The framework makes you address all four on purpose.",
  scriptBeats: [
    { cue: "Slide 1 — Title: 'The four layers of interoperability'. Voice-over begins." },
    { text: "When two government systems fail to work together, people usually assume the problem is technical — a missing connection, an old system. Sometimes it is. More often, the wires are fine and the exchange still fails, because interoperability is not one thing. It is four. The European Interoperability Framework, and the X-Road systems built on it, describe interoperability as four layers stacked together. Skip any one, and the exchange breaks at that layer." },
    { cue: "Slide 2 — Title: 'Four layers, bottom to top'. Body, four stacked text rows: 'Technical — the wires and the bus.' 'Semantic — the meaning of the data.' 'Organisational — who agrees to exchange, and how it is governed.' 'Legal — the mandate that makes it lawful.'" },
    { text: "Picture the four layers stacked. At the bottom, technical — the wires, the bus, the message format. Above it, semantic — what the data actually means. Above that, organisational — who agrees to exchange data with whom, and who governs the arrangement. And at the top, legal — the mandate that makes the whole exchange lawful. A real exchange needs all four. Let us take them one at a time, from the one people think about first." },
    { cue: "Slide 3 — Title: 'Technical — necessary, not sufficient'. Body, two text rows: 'A shared bus and shared standards, so systems can connect without a new link each time.' 'X-Road is one such bus. But a working wire that carries the wrong meaning is still a failure.'" },
    { text: "The technical layer is the shared bus and the standards every member uses — so that connecting is a one-time setup, not a new project each time. X-Road is one such bus. This layer is necessary. But it is not sufficient, and it is not even the hardest part. A perfectly working wire that delivers data the receiving agency misreads is still a failed exchange." },
    { cue: "Slide 4 — Title: 'Semantic — the shared language'. Body, two text rows: 'The agriculture ministry's 'farmer' and the bank's 'farmer' must mean the same person, or the data is worse than useless.' 'Agreed data models, code lists and identifiers are what make meaning travel.'" },
    { text: "The semantic layer is meaning. The agriculture ministry's record of a 'farmer' and the cooperative bank's record of a 'farmer' must point to the same person, identified the same way, or combining them does harm. This is where business and IT must speak a shared language — because agreeing what 'learner' or 'farmer' or 'enrolment' means is a business decision that only the people who own the data can make, written down precisely enough for the systems to use. The framework gives both sides that shared vocabulary. Without it, the wire carries confident nonsense." },
    { cue: "Slide 5 — Title: 'Organisational — who agrees, who governs'. Body, three text rows: 'Which agency provides which data, to whom, for what purpose.' 'Service-level expectations: uptime, response, support.' 'A governing body that admits members and resolves disputes.'" },
    { text: "The organisational layer is agreement and governance. Which agency provides which data, to which other agency, for what purpose, at what level of service. Who is accountable when an exchange fails. Who admits a new member to the bus and who resolves a dispute between two members. This is the layer where the minister and the chief architect sit at the same table — the business side deciding what may be shared and why, the IT side deciding how — and where a governing body turns a set of bilateral favours into a dependable federation." },
    { cue: "Slide 6 — Title: 'Legal — the mandate'. Body, two text rows: 'A decree or law that makes cross-agency exchange lawful, makes connection mandatory, and protects the citizen's data.' 'Without it, every exchange is a legal risk one official can veto.'" },
    { text: "And the legal layer is the mandate. A decree or a law that makes cross-agency exchange lawful in the first place, that makes connecting to the bus mandatory rather than optional, and that protects the citizen's data as it moves. Without the legal layer, every exchange is a standing legal risk that one cautious official can stop. With it, exchange is the default and refusing to connect is the exception that needs justifying. The decree is not paperwork. It is the on-switch." },
    { cue: "Slide 7 — Title: 'The three seams'. Body, three text rows: 'Legal authorises organisational — no decree, no authority to set standards or sanction.' 'Organisational governs technical and semantic — the operating authority publishes the standards and curates the vocabulary.' 'Technical and semantic make the legal obligations enforceable — once-only cannot be enforced until the bus and the shared meaning exist.'" },
    { text: "The four layers are not four separate projects — they lock together at three seams. The legal layer authorises the organisational one: without a decree there is no authority to set a binding standard or to sanction an agency that ignores it. The organisational layer governs the technical and semantic ones: the operating authority is who publishes the standards and curates the shared vocabulary. And the technical and semantic layers are what make the legal obligations enforceable: a once-only rule on paper cannot be enforced until the bus runs and the two agencies mean the same thing. That is why the layers are built in parallel, never technical first and legal later." },
    { cue: "Slide 8 — Title: 'In one sentence'. Body, large text (Arial Bold 28pt): 'Interoperability is four layers — technical, semantic, organisational, legal. It fails at whichever one you skip.'" },
        { text: "So the discipline is simple to state and hard to skip: four layers, built in parallel, and a failure at whichever one you neglect. Most failed exchanges had working wires." },
    { cue: "Slide 9 — Title: 'Sources'. Body: EU European Interoperability Framework (four-layer model); NIIS X-Road documentation; PAERA v1.0 §3.4.3. Footer: 'Find the link in the description.'" }
  ],
  slideSpecRows: [
    ["1", "Title slide. Title: 'The four layers of interoperability'.",
      "Standard ITU template. No images."],
    ["2", "Four-layer stack. Four text rows bottom-to-top (Technical / Semantic / Organisational / Legal), one short gloss each.",
      "The spine of the whole module. A plain stacked text diagram — text-box rows, no icons."],
    ["3", "Technical layer slide. Two text rows: what it is; why it is necessary but not sufficient.",
      "Deflates the 'it's just a technical problem' assumption."],
    ["4", "Semantic layer slide. Two text rows: the same-person problem; what makes meaning travel.",
      "Carries the shared-language argument — business and IT must agree meaning together."],
    ["5", "Organisational layer slide. Three text rows: data-sharing agreements, service levels, a governing body.",
      "Sets up Module 3 (governance). Text-only."],
    ["6", "Legal layer slide. Two text rows: the mandate; the cost of skipping it. 'The decree is the on-switch.'",
      "Sets up Module 2 (the decree). The 'on-switch' line is the memorable beat."],
    ["7", "Three-seams slide. Three text rows: legal authorises organisational; organisational governs technical and semantic; technical and semantic make legal enforceable.",
      "The interaction between the layers — why they are built in parallel. Text-only."],
    ...
    ["9", "Sources slide. Footer: 'Find the link in the description.'",
      "Lets viewers verify the four-layer model is from EIF/NIIS, not invented."]
  ],
  aiTip: {
    title: "Map a planned exchange across the four layers — and find the missing one",
    problem: "A Strategist scoping a cross-agency exchange needs to check it is covered at all four layers, not just the technical one — the single most common cause of stalled interoperability. This prompt produces a four-layer readiness check for one exchange.",
    ...
    io: "Input: a 2-4 sentence description of one planned exchange. Output: a four-row layer-readiness table plus the binding-constraint call.",
...
body.push(...renderSubtopic({
  ...
  title: "The once-only promise",
  runtime: "~4 min",
  words: 520,
  paeraAnchor: "PAERA §5.2 Principle #5 (Once-Only); EU Once-Only Technical System (OOTS); EU EIF",
  singleMessage: "Once-only — the state never asks a citizen for the same data twice — is the outcome interoperability is for, and the test of whether it works.",
  scriptBeats: [
    { cue: "Slide 1 — Title: 'The once-only promise'. Voice-over begins." },
    { text: "It is easy to talk about interoperability in terms of buses and standards and lose sight of what it is for. So here is the outcome, in one sentence a citizen would recognise: the state never asks you for the same information twice. Give your identity once, and every service that is entitled to it can get it — with your consent and a lawful basis — without asking you again. This is the once-only principle, and it is the single clearest test of whether your interoperability framework actually works." },
    { cue: "Slide 2 — Title: 'The citizen's day, without once-only'. Body, single text block: 'She proves her identity at the ID office. She proves it again at the school to enrol her child. She proves it again at the clinic. The same document, the same queue, three times — because the agencies cannot ask each other.'" },
    { text: "Picture a citizen's day without it. She proves her identity at the identity office. She proves it again, on paper, when she enrols her child at school. She proves it a third time at the clinic. Same document, same queue, three times — not because anyone wants to harass her, but because each agency has no way to ask the agency that already holds the answer. The burden is pushed onto the person with the least power to carry it." },
    { cue: "Slide 3 — Title: 'What once-only actually requires'. Body, three text rows: 'A trusted source for each fact — one authoritative holder.' 'A lawful basis and the citizen's consent to share it.' 'A real-time way to fetch it at the moment of service.'" },
    { text: "Once-only sounds simple, but it requires all four layers we just named, working together. It requires a trusted source for each fact — one authoritative holder of identity, one of enrolment, rather than five copies that disagree. It requires a lawful basis and, where appropriate, the citizen's consent. And it requires a real-time way to fetch the fact at the moment of service, over the bus, instead of asking the citizen to carry it. Once-only is not a feature you switch on. It is what you get when the four layers are built correctly." },
    { cue: "Slide 4 — Title: 'Once-only in Progressa'. Body, three text rows: 'A learner applies for a credential at the examination authority.' 'Instead of re-asking, the authority pre-fills identity from the identity authority and enrolment from the learner registry — over the bus.' 'The learner is asked once. The data is fetched, with consent, in seconds.'" },
    ...
    { cue: "Slide 5 — Title: 'Why once-only is the right north star'. Body, two text rows: 'It is measurable — count the times a citizen is asked for data the state already holds.' 'It keeps the framework honest — a bus nobody uses for a real once-only exchange is not yet working.'" },
    { text: "Make once-only your north star, for two reasons. It is measurable — you can literally count the number of times a citizen is asked for data the state already holds, and drive that number down. And it keeps the framework honest. It is possible to build a technically impressive bus that no real service uses. Once-only is the test that cuts through that: until a real service fetches a real fact for a real citizen who is no longer asked twice, the framework is plumbing, not yet a result." },
    { cue: "Slide 6 — Title: 'In one sentence'. Body, large text (Arial Bold 28pt): 'Once-only is the promise to the citizen and the test of the framework: ask once, fetch the rest — lawfully, with consent, over the bus.'" },
    ...
    { cue: "Slide 7 — Title: 'Sources'. Body: PAERA v1.0 §5.2 Principle #5 (Once-Only); EU Once-Only Technical System (OOTS); EU EIF. Footer: 'Find the link in the description.'" }
  ],
  slideSpecRows: [
    ["1", "Title slide. Title: 'The once-only promise'.",
      "Standard ITU template. No images."],
    ["2", "Citizen's day slide. Single text block describing the same proof demanded three times.",
      "The empathy beat — concrete, citizen-level. Text-only."],
    ["3", "What once-only requires. Three text rows: trusted source, lawful basis/consent, real-time fetch.",
      "Connects the outcome back to the four layers without re-teaching them."],
    ["4", "Progressa once-only. Three text rows: the learner, the pre-fill from PNIA and PLR, asked once.",
      "Previews the Module 5 demonstration. Uses the bound Progressa institutions."],
    ["5", "Why once-only is the north star. Two text rows: measurable; keeps the framework honest.",
      "Reframes once-only as a management metric, not a slogan."],
    ["6", "Single-sentence summary slide. One large text block (Arial Bold 28pt).",
      "The quotable line for a ministerial briefing."],
    ["7", "Sources slide. Footer: 'Find the link in the description.'",
      "Lets viewers verify the once-only references."]
  ],
  aiTip: {
    title: "Find your country's highest-value once-only exchange",
    problem: "A Strategist needs to pick the first once-only exchange to build — the one that removes the most citizen burden for the least effort. This prompt produces a ranked shortlist that motivates the framework with a concrete first win.",
    ...
    io: "Input: a list of services and the data each demands at intake. Output: a ranked once-only opportunity table plus a first-exchange recommendation.",
...
body.push(...renderSubtopic({
  ...
  title: "The Strategic Foundation Document",
  runtime: "~4 min",
  words: 560,
  paeraAnchor: "EU EIF (interoperability strategy & governance); PAERA §3.4.3; §5.2 (Whole-of-Government, Once-Only, Cross-border)",
  singleMessage: "Name the mandate, the scope and the principles once, in writing, before any wiring — it is what every later decision is checked against.",
  scriptBeats: [
    { cue: "Slide 1 — Title: 'The Strategic Foundation Document'. Voice-over begins." },
    { text: "Before a single system is connected, before the decree is drafted, before the first agency joins the bus, there is one document to write. It is short — a handful of pages, not a tome. It is the Strategic Foundation Document, and its job is to answer the questions that every later decision will be tested against. Skip it, and every meeting re-opens the same arguments. Write it, and you have a reference you can point to when the arguments come — and they will." },
    { cue: "Slide 2 — Title: 'What it answers'. Body, four text rows: 'Mandate — by whose authority does this framework exist?' 'Scope — which agencies and which exchanges are in, and which are not, yet?' 'Principles — the rules every design decision must honour.' 'Success — how we will know it is working.' Footer: 'On paper, the full document has seven sections: policy intent, sectoral demand, Tier 1 commitments, target benefits, objectives, political sponsorship, next steps.'" },
    { text: "The document answers four questions. By whose authority does this framework exist — the mandate? Which agencies and which exchanges are in scope, and just as importantly, which are explicitly out of scope for now? What principles must every design decision honour? And how will we know it is working — what does success look like in eighteen months? Four questions. Answer them on paper, once, with the people who can make the answers stick. On paper, the full template runs to seven sections — policy intent, the sectors that want it first, the Tier 1 commitments, the target benefits, the objectives for two years and five, the political sponsorship, and the next steps — but they all serve those four questions." },
    { cue: "Slide 3 — Title: 'The principles to name'. Body, three text rows: 'Whole-of-government — build the shared road once, reuse it everywhere.' 'Once-only — ask the citizen once.' 'Cross-border and reuse-first — prefer published standards and shared building blocks over bespoke.'" },
    { text: "The principles are the heart of the document, because they settle arguments before they start. Name whole-of-government — we build the shared road once and reuse it, rather than each agency building its own. Name once-only — we ask the citizen once. Name reuse-first and cross-border readiness — we prefer published standards and shared building blocks over bespoke ones, and we design so that an exchange could one day cross a border. These are not slogans on a wall. They are the criteria you will hold the next design decision to, and the next procurement, and the next request for an exception." },
    { cue: "Slide 4 — Title: 'Why write it before the wiring'. Body, two text rows: 'Every technical and legal choice later is checked against it — it is the constitution of the framework.' 'It is what a new minister, a new CIO or a new donor reads to understand the framework in ten minutes.'" },
    { text: "Why write it first, before the interesting technical work? Two reasons. It becomes the constitution of the framework — when a later choice about a standard, or a member, or an exception comes up, you check it against the foundation rather than re-litigating first principles in a technical meeting. And it is the on-ramp for everyone who joins later. A new minister, a new agency CIO, a new development partner can read these few pages and understand what the framework is, what it covers, and what it stands for — in ten minutes, not ten meetings." },
    { cue: "Slide 5 — Title: 'Keep it short, keep it owned'. Body, two text rows: 'A handful of pages, signed by the authority that mandates the framework.' 'Living, not frozen — reviewed when scope expands, but stable enough to be a reference.'" },
    { text: "Two cautions. Keep it short — a handful of pages that a busy minister will actually read and sign. And keep it owned — it carries the signature of the authority that mandates the framework, so that pointing to it carries weight. It is a living document, reviewed when the scope expands to new agencies or new exchanges, but it is stable enough to be the thing everyone points back to. The Strategic Foundation Document is the first artefact in your build pack, and the one every other artefact answers to." },
    { cue: "Slide 6 — Title: 'In one sentence'. Body, large text (Arial Bold 28pt): 'Write the mandate, scope, principles and success measures on a few pages first — so every later decision has something to be checked against.'" },
    { text: "So the first move, before any wiring, is this document. Write the mandate, the scope, the principles and the success measures on a few pages, get them signed by the right authority, and put them where everyone can find them. It is the cheapest insurance you will buy in the whole programme — because it is what every later, more expensive decision gets checked against." },
    { cue: "Slide 7 — Title: 'Sources'. Body: EU European Interoperability Framework (interoperability strategy & governance); PAERA v1.0 §3.4.3; §5.2. Footer: 'Find the link in the description.'" }
  ],
  slideSpecRows: [
    ["1", "Title slide. Title: 'The Strategic Foundation Document'.",
      "Standard ITU template. No images."],
    ["2", "Four-question slide. Four text rows: mandate, scope, principles, success.",
      "The structure of the document, in four words. Text-only."],
    ["3", "Principles slide. Three text rows: whole-of-government, once-only, reuse-first/cross-border.",
      "Reinforces the structural argument (planning enables re-use) as a named principle."],
    ["4", "Why-first slide. Two text rows: the constitution; the ten-minute on-ramp.",
      "Justifies the sequencing — foundation before wiring."],
    ["5", "Short-and-owned slide. Two text rows: keep it short; keep it signed and living.",
      "Practical cautions. Names it as the first build-pack artefact."],
    ["6", "Single-sentence summary slide. One large text block (Arial Bold 28pt).",
      "The take-home line."],
    ["7", "Sources slide. Footer: 'Find the link in the description.'",
      "Lets viewers verify the EIF and PAERA references."]
  ],
  aiTip: {
    title: "Draft your Strategic Foundation Document",
    problem: "A Strategist starting an interoperability framework needs a first draft of the foundation document to circulate — short, signed-off-able, and tailored to the country. This prompt produces that draft from a short brief.",
    ...
    io: "Input: a 1-3 paragraph brief on mandate, first-wave agencies, first exchanges and constraints. Output: a ~2-page Strategic Foundation Document draft in four sections.",
...
body.push(...renderSubtopic({
  ...
  aiTip: {
    title: "Build your Use-Case Catalogue from your integration map",
    problem: "A Strategist with an integration map (or a list of desired exchanges) needs to turn it into a ranked catalogue that names the citizen value and the readiness of each exchange. This prompt produces the first catalogue draft.",
    ...
    io: "Input: an integration map or list of desired exchanges. Output: a Use-Case Catalogue table plus a ranked first-wave shortlist of three.",
...
body.push(...renderSubtopic({
  ...
  title: "Mapping your stakeholders",
  runtime: "~4 min",
  words: 580,
  paeraAnchor: "EU EIF (organisational layer, governance); NIIS X-Road governance model",
  singleMessage: "Sort the agencies into Champions, Early Adopters and Observers, and you know who to onboard first and who to convince.",
  scriptBeats: [
    { cue: "Slide 1 — Title: 'Mapping your stakeholders'. Voice-over begins." },
    { text: "Interoperability is often described as a technical problem. It is at least as much an agreement problem. A bus with no members carries nothing. So before you build, you map the people whose agreement you need — the agencies who will provide data, the agencies who will consume it, and the leaders whose support or resistance will decide whether the framework lives. This map is the third foundation artefact, and it is what turns a good design into a thing that actually gets adopted." },
    { cue: "Slide 2 — Title: 'Three tiers'. Body, three text rows: 'Champions — agencies that want this now and will go first.' 'Early Adopters — willing once they see it work.' 'Observers — not yet convinced, watching from the side.'" },
    { text: "Sort the agencies into three tiers. Champions are the agencies that already feel the pain and want the exchange now — they will go first, and their willingness is your most valuable asset. Early Adopters are willing, but want to see it work before they commit their own systems and staff. Observers are not yet convinced — they are watching from the side, and some are quietly hoping it fails so they need not change. You will treat each tier differently, and knowing which is which is half the battle." },
    { cue: "Slide 3 — Title: 'Provider and consumer — the interoperability twist'. Body, two text rows: 'Every exchange has a data provider and a data consumer — and they are different agencies with different incentives.' 'The consumer wants the exchange; the provider carries the cost and the risk. Map both sides for every exchange.'" },
    { text: "Interoperability adds a twist that other digital programmes do not have. Every exchange has two sides — an agency that provides the data and an agency that consumes it — and they have different incentives. The consumer usually wants the exchange; it makes their service easier. The provider often carries the cost and the risk — the load on their system, the responsibility if the data is wrong. So for each exchange in your catalogue, map both sides: who gains and who is asked to give. A provider agency that sees only cost and no benefit is where adoption stalls, and where you will need your minister's authority and a fair service agreement." },
    { cue: "Slide 4 — Title: 'Who to work first'. Body, four text rows: 'Pair a Champion consumer with a willing provider — that is your first exchange.' 'Use the first win to move Early Adopters.' 'Bring Observers along with evidence, not argument.' 'Lock the Champions in writing: a one-day inception workshop, then a signed commitment letter from each.'" },
    { text: "The map tells you the sequence. Find a Champion consumer paired with a willing provider, and that is your first exchange — the one most likely to succeed and prove the framework. Use that first working exchange to move the Early Adopters, who needed to see it before committing. And bring the Observers along with evidence rather than argument — a working once-only exchange that citizens notice is more persuasive than any number of strategy meetings. You spend your scarce political capital where it moves the most agencies. And lock the Champions in before you build on them: a one-day inception workshop with the Tier 1 ministries, ending in a signed commitment letter from each, a named focal point and an agreed quick win. Programmes that skip the signature stall at the first cross-ministry decision, when a Champion turns out to have been a well-wisher." },
    { cue: "Slide 5 — Title: 'A bus needs an owner'. Body, single text block: 'None of this holds together without a body that governs the framework — admits members, sets service levels, resolves disputes. An interoperability platform with no owner decays in its second year. Governance is the next module.'" },
    { text: "One warning the stakeholder map makes visible. All of this — the tiers, the providers and consumers, the agreements — needs a body that governs the framework as a whole: that admits new members, sets the service levels, and resolves disputes when two agencies disagree. An interoperability platform with no owner decays in its second year, when the founding enthusiasm fades and no one is accountable for keeping it healthy. Designing that governing body is the subject of a later module. The stakeholder map is what shows you why it is not optional." },
    { cue: "Slide 6 — Title: 'In one sentence'. Body, large text (Arial Bold 28pt): 'Map the agencies into Champions, Early Adopters and Observers — and map provider against consumer for every exchange — so you spend authority where it moves the most agencies.'" },
    { text: "So the third foundation artefact is the stakeholder map. Three tiers — Champions, Early Adopters, Observers. Two sides for every exchange — provider and consumer. Together they tell you who to onboard first, who to convince with evidence, and where you will need your minister to lean in. Interoperability is an agreement problem, and this map is how you solve it deliberately instead of hoping the agencies come along on their own." },
    { cue: "Slide 7 — Title: 'Sources'. Body: EU European Interoperability Framework (organisational layer); NIIS X-Road governance model (niis.org). Footer: 'Find the link in the description.'" }
  ],
  slideSpecRows: [
    ["1", "Title slide. Title: 'Mapping your stakeholders'.",
      "Standard ITU template. No images."],
    ["2", "Three-tier slide. Three text rows: Champions, Early Adopters, Observers.",
      "The core model. Text-only."],
    ["3", "Provider-vs-consumer slide. Two text rows on the two sides and their different incentives.",
      "The interoperability-specific insight. Sets up fair service agreements (Module 3/5)."],
    ["4", "Sequencing slide. Four text rows: first exchange, move Early Adopters, evidence for Observers, lock the Champions in writing.",
      "Turns the map into an onboarding sequence."],
    ["5", "Bus-needs-an-owner slide. Single text block previewing governance.",
      "Sets up Module 3. The 'decays in its second year' line is the memorable beat."],
    ["6", "Single-sentence summary slide. One large text block (Arial Bold 28pt).",
      "The take-home line."],
    ["7", "Sources slide. Footer: 'Find the link in the description.'",
      "Lets viewers verify the EIF/NIIS references."]
  ],
  aiTip: {
    title: "Map your stakeholders into tiers and provider/consumer roles",
    problem: "A Strategist needs to turn a list of agencies into an actionable stakeholder map — who is a Champion, who is an Observer, and who provides versus consumes for the first exchanges. This prompt produces that map and an onboarding sequence.",
    ...
    io: "Input: a list of agencies with their data roles and attitudes. Output: a tier map, a provider/consumer table, a recommended onboarding sequence, and a one-paragraph commitment-letter template for the Tier 1 ministries.",
...
body.push(...renderSubtopic({
  ...
  title: "What the world already proved",
  runtime: "~4 min",
  words: 520,
  paeraAnchor: "Estonia X-Road & EU OOTS (interoperability exemplars); education semantic standards; PAERA §3.4.3",
  singleMessage: "Estonia's X-Road and the EU's once-only systems show the pattern; the education semantic standards show the vocabularies you will reuse rather than invent.",
  scriptBeats: [
    { cue: "Slide 1 — Title: 'What the world already proved'. Voice-over begins." },
    { text: "You are not the first country to build this, and that is good news. The pattern in this knowledge product is not a theory — it is drawn from systems that have run for years, at national and cross-border scale. Knowing the evidence does two things for you. It gives your minister confidence that the approach is proven. And it tells your team which parts to reuse rather than reinvent — because the most expensive mistake in interoperability is rebuilding a standard that already exists." },
    { cue: "Slide 2 — Title: 'Estonia — X-Road, the data-exchange backbone'. Body, three text rows: 'A shared bus connecting hundreds of agencies, run for two decades.' 'Distributed registries — each fact owned by one accountable agency.' 'Once-only as the law, not the aspiration.'" },
    { text: "The clearest example is Estonia's X-Road. Starting in the early 2000s, Estonia built a shared data-exchange layer that now connects hundreds of public and private organisations. Two design choices made it last. Registries are distributed — each fact has one accountable owner, rather than copies scattered across agencies. And once-only is built into law, not left as an aspiration. The technical bus your country will use — the one we demonstrate later on the Linkup federation — is from this same X-Road lineage. Estonia is a small state with different starting conditions, so use it as a proof that the pattern works, not as a template to copy line for line." },
    { cue: "Slide 3 — Title: 'The EU — once-only across borders'. Body, two text rows: 'The Once-Only Technical System lets a citizen of one country use official data held in another.' 'If once-only can work across 27 countries, it can work across your ministries.'" },
    { text: "The second example raises the ambition. The European Union built the Once-Only Technical System so that a citizen or business dealing with one member state can have official evidence fetched from another member state, with consent, instead of carrying paper across borders. The point for you is not the cross-border machinery itself. It is the proof of scale: if once-only can be made to work across twenty-seven countries with different languages, laws and systems, then making it work across the ministries of a single country is an easier problem than it looks — provided you build the four layers properly." },
    { cue: "Slide 4 — Title: 'Reuse the vocabulary, don't invent it'. Body, four text rows: 'Giga — a shared way to identify and locate schools.' 'OneRoster — a standard for rostering and enrolment data.' 'CEDS — a common vocabulary for education data.' 'Europass / W3C Verifiable Credentials — a standard for digital credentials.'" },
    { text: "The third kind of evidence is the most practical. For the education sector, the semantic layer — the shared meaning of the data — is largely already published. Giga gives a shared way to identify and locate schools. OneRoster is a standard for rostering and enrolment data. CEDS offers a common vocabulary for education data elements. And Europass, with W3C Verifiable Credentials, is a published standard for digital credentials a learner can carry and a verifier can trust. Your team does not invent what a 'school' or a 'credential' is. They adopt these vocabularies, and spend their scarce time on what is genuinely specific to your country. That is the reuse-first principle made concrete." },
    { cue: "Slide 5 — Title: 'The pattern travels'. Body, two text rows: 'A shared bus, distributed registries, once-only in law, reused standards.' 'Different countries, different scale — the same four elements, every time.'" },
    { text: "Look across the evidence and the same four elements appear every time. A shared bus rather than point-to-point links. Distributed registries with accountable owners. Once-only written into law. And standards reused rather than reinvented. Different countries, very different scale — the same pattern. It travels because it is built on how the problem actually works, not on any one country's circumstances. Your country can apply it too." },
    { cue: "Slide 6 — Title: 'In one sentence'. Body, large text (Arial Bold 28pt): 'The pattern is proven and the vocabularies are published — your team's job is to apply them to your country, not to invent them.'" },
    ...
    { cue: "Slide 7 — Title: 'Sources'. Body: Estonia — e-Estonia.com, RIA (ria.ee), X-Road (niis.org); EU Once-Only Technical System; Giga; OneRoster (1EdTech); CEDS; Europass / W3C Verifiable Credentials. Footer: 'Find the link in the description.'" }
  ],
  slideSpecRows: [
    ["1", "Title slide. Title: 'What the world already proved'.",
      "Standard ITU template. No images."],
    ["2", "Estonia slide. Three text rows: shared bus, distributed registries, once-only in law.",
      "Country name in plain typography, no national emblem. Polestar framing — proven, different context."],
    ["3", "EU OOTS slide. Two text rows: cross-border once-only; the scale argument.",
      "The ambition-raising beat. Text-only."],
    ["4", "Reuse-the-vocabulary slide. Four text rows naming the education semantic standards.",
      "The most practical slide — concrete standards to adopt. No logos, plain text names."],
    ["5", "Pattern-travels slide. Two text rows: the four recurring elements; different scale, same pattern.",
      "The synthesis. Reinforces reuse-first."],
    ["6", "Single-sentence summary slide. One large text block (Arial Bold 28pt).",
      "The take-home line, and the bridge into the rest of KP2."],
    ["7", "Sources slide. Bulleted text across Estonia, EU, and the education standards. Footer: 'Find the link in the description.'",
      "Lets viewers verify each example and standard via the description."]
  ],
  aiTip: {
    title: "Find the published standards your sector should reuse",
    problem: "A Strategist wants to avoid reinventing semantic standards that already exist for their sector. This prompt produces a shortlist of published standards to adopt for a given sector and country, with what each covers.",
    prompt: "My interoperability work is in the [education / health / agriculture / social protection] sector in [country X]. List the established, publicly published standards and reference vocabularies my team should consider adopting rather than inventing, for: (1) identifying the core entities in this sector (e.g. schools, learners, facilities, beneficiaries); (2) the main data records exchanged; (3) credentials or certificates issued; (4) any sector-specific code lists or classifications. For each standard: its name, the body that maintains it, what it covers in one line, and how mature/widely adopted it is. Flag where two standards overlap or compete so I can choose. Prefer open, internationally recognised standards, and note where a regional or national profile would still be needed on top. Output: a table (standard / maintainer / what it covers / maturity / note), plus a 3-bullet 'adopt first' recommendation.",
    io: "Input: the sector and country. Output: a table of candidate standards plus an 'adopt first' shortlist.",
...
body.push(
  ...
  P("Quantitative and historical claims that are defensible at the chosen level of generality but should be web-verified before final lock: the Estonia X-Road origin date and build-out timeline (1.1, 1.7); the EU Once-Only Technical System / OOTS as the cross-border once-only benchmark (1.3, 1.7); the characterisation of the education semantic standards — Giga, OneRoster, CEDS, Europass / W3C Verifiable Credentials (1.2, 1.7). Cost and timeline figures benchmarked to Estonia's X-Road are deferred to Topic 5 (implementation) but flagged here for consistency."),

  H3("5.2 Editorial tone calls"),
  P("Sharp lines that deserve a deliberate keep / soften / cut decision: 'procurement rules can require behaviour, they cannot make it the cheapest choice' (1.1); 'an interoperability platform with no owner decays in its second year' (1.6, previewing governance); 'the decree is not paperwork, it is the on-switch' (previewed in 1.1, developed in Topic 2)."),

  H3("5.3 Structural calls"),
  P("(1) Progressa demonstration membership. The Inception Report §6.2 names the Topic-5 demonstration as 'PEMIS ↔ PLR ↔ PDCA ↔ PNIA', but §4.3 lists the four Linkup Security Servers as MoEYS/PEMIS, PNEA, PLR and PNIA — with PDCA being the Digital Credentials Authority used in KP4. This bundle binds to the §4.3 four-server canon (MoEYS/PEMIS, PNEA, PLR, PNIA, Central Server at PDGA). Confirm with ITU; if PDCA is wanted in the KP2 demonstration it becomes a fifth Security Server. (2) The proposed once-only worked flow for Topic 5 — PNEA pre-filling identity from PNIA and enrolment from PLR — is previewed in 1.3 and should be confirmed as the single most compelling once-only story before the demonstration is built."),

  H3("5.4 Dependencies"),
  P("The Topic-5 live demonstration and the build pack depend on the full Linkup federation being live on ITU cloud (Inception Report action item A4) and on ITU's video look-and-feel template (action item A5). The schedule for taking KP2 to full depth — modules plus the runnable four-system pack — should be settled in the re-phasing discussion with ITU (see the KP2–4 Delivery Plan §6)."),
...
body.push(
  ...
  P("Compiled across the seven subtopics for ITU's video production pipeline. To be split per subtopic and inserted into the corresponding YouTube descriptions."),
...
const doc = new Document({
  ...
  title: "KP2 Module 1 — Video Script Bundle v0.2 (ITU-aligned)",
  description: "Video script bundle for KP2 Topic 1 (Government Interoperability Framework), aligned to ITU's Knowledge Products and Video Materials Guide.",
  styles: {
  ...
  sections: [{
    ...
    headers: { default: new Header({ children: [new Paragraph({ alignment: AlignmentType.RIGHT,
      children: [new TextRun({ text: "FiscalAdmin OÜ — ITU/Giga · KP2 Topic 1 Script Bundle v0.2 · 12 September 2026",
...
Packer.toBuffer(doc).then(buf => {
  const out = process.env.OUT_PATH || path.join(__dirname, "KP2_Module1_Script_Bundle_v0.2.docx");
