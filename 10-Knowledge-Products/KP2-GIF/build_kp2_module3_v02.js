// Build KP2 Module 3 — Video Script Bundle v0.2
// v0.2 (12 Sep 2026): aligned to the 08-Interoperability source (Method Step 4; Reference Model §5; Reference Architecture
//   §4.4, §4.7). 3.1 teaches the regulator/operator split as the recommended pattern (rules vs running; a small administration
//   makes them two units in one agency), the three organisational forms and the staffing scale. 3.3 carries the split's
//   RACI rows. 3.4 adds the DPO, the Technical Focal Point and cost-sharing. 3.5 names the Working Groups as the source does.
//   3.6 adds the conformance regime and the semantic registry as standing assets.
...
body.push(
  ...
  genericTable([700, 2700, 4700, 1600], ["#", "Title", "Single message", "Runtime"], [
    ["3.1", "Why a bus needs an owner",
      "An interoperability platform with no owner decays in its second year — name the owner before the first member joins, and split the body that sets the rules from the body that runs the bus.", "~5 min"],
    ...
    ["3.5", "The four Technical Working Groups",
      "Security, semantics, APIs, platform operations — the four standing groups that keep the bus coherent as it grows.", "~4 min"],
...
body.push(...renderSubtopic({
  ...
  singleMessage: "An interoperability platform with no owner decays in its second year — name the owner before the first member joins, and split the body that sets the rules from the body that runs the bus.",
  scriptBeats: [
    ...
    { cue: "Slide 3 — Title: 'Two hats — the regulator and the operator'. Body, three text rows: 'The regulator owns the architecture, the standards, the compliance checks and the sanctions.' 'The operator runs the platform, onboards members, validates connections, supports them.' 'No single body both runs the bus and judges compliance with it — in a small administration, two units inside one agency, each with its own mandate.'" },
    { text: "The duties on the last slide are really two hats, and the most useful thing you can do at the start is to put them on two heads. One hat sets the rules: it owns the reference architecture and the standards, it defines the compliance checks and the evidence, it grants written exceptions and applies sanctions. That is the regulator. The other hat runs the bus: it operates the platform, onboards members, validates their connections and supports them. That is the operator. When one office does both, a platform outage, a missed standard and a compliance waiver are all decided by the same people, with no independent check — and the waiver always wins. So write the split into both mandates, so it survives staff turnover. A small administration does not need two agencies; two units inside one agency, each with its own mandate, is enough. And the member ministries keep the third hat: they own their services and their data, and they answer for their own compliance." },
    { cue: "Slide 4 — Title: 'Governing is not operating'. Body, two text rows: 'The governing tiers set direction and make decisions.' 'The Operating Authority runs the platform day to day. A framework with governance but no operator is a set of meetings with nothing underneath.'" },
    ...
    { cue: "Slide 5 — Title: 'In one sentence'. Body, large text (Arial Bold 28pt): 'A shared bus needs a named owner with budget, staff and mandate before the first member joins — and the body that sets the rules is not the body that runs the bus.'" },
    { text: "So the first governance decision is the simplest to state and the easiest to skip: name the owner, with budget, staff and a real mandate, before the first member joins — and keep the rule-maker and the operator apart. That is what stops the second-year decay." },
    { cue: "Slide 6 — Title: 'Sources'. Body: EU EIF / NIIS X-Road governance; Estonia model (Ministry of Economic Affairs and Communications + RIA); the regulator/operator split (the framework's reference architecture); PAERA v1.0 §3.1.3. Footer: 'Find the link in the description.'" }
  ],
  slideSpecRows: [
    ...
    ["2", "What-the-owner-does slide. Five text rows of duties.",
      "The core role definition. Text-only list."],
    ["3", "Two-hats slide. Three text rows: the regulator, the operator, the rule that no single body does both.",
      "The regulator/operator split — the recommended pattern. Text-only."],
    ["4", "Governing-vs-operating slide. Two text rows distinguishing the two.",
  ...
  aiTip: {
    title: "Define the owner's mandate — and split the regulator from the operator",
    ...
    io: "Input: the candidate host body and its current mandate. Output: a mandate statement with the regulator and operator hats separated, plus a list of mandate gaps.",
...
body.push(...renderSubtopic({
  ...
  slideSpecRows: [
    ...
    ["3", "Decisions-to-map slide. Eight text rows of recurring decisions.",
...
body.push(...renderSubtopic({
  ...
  slideSpecRows: [
    ["1", "Title slide. Title: 'Member obligations'.",
      "Standard ITU template. No images."],
    ["2", "What-every-member-accepts slide. Eight text rows of common obligations.",
...
body.push(...renderSubtopic({
  ...
  singleMessage: "Security, semantics, APIs, platform operations — the four standing groups that keep the bus coherent as it grows.",
  scriptBeats: [
    ...
    { cue: "Slide 2 — Title: 'Four groups, four dimensions'. Body, four text rows: 'Security — the trust model, certificates, access control.' 'Semantics — shared data models, code lists, identifiers.' 'APIs — the technical standards, the service contracts, the message formats.' 'Platform operations — running the bus, and the process to admit and connect a member.'" },
    ...
    { cue: "Slide 5 — Title: 'In one sentence'. Body, large text (Arial Bold 28pt): 'Four standing groups — security, semantics, APIs, platform operations — each owning one technical dimension, are what keep the bus one framework as it grows.'" },
...
body.push(...renderSubtopic({
  ...
  slideSpecRows: [
    ...
    ["4", "Standing-assets slide. Two text rows: the conformance regime, the semantic registry.",
      "The two standing mechanisms the source method adds to change control. Text-only."],
  ...
  aiTip: {
    ...
    io: "Input: the framework's governance bodies and RACI. Output: a change-control process, a standards-portfolio register template with its conformance fields, and a one-paragraph charter for the semantic registry (steward, owners, what it holds).",
...
body.push(
  ...
  H3("5.5 The regulator/operator split (v0.2)"),
...
const doc = new Document({
  ...
  title: "KP2 Module 3 — Video Script Bundle v0.2 (ITU-aligned)",
  ...
  sections: [{
    ...
    headers: { default: new Header({ children: [new Paragraph({ alignment: AlignmentType.RIGHT,
      children: [new TextRun({ text: "FiscalAdmin OÜ — ITU/Giga · KP2 Topic 3 Script Bundle v0.2 · 12 September 2026",
...
Packer.toBuffer(doc).then(buf => {
  const out = process.env.OUT_PATH || path.join(__dirname, "KP2_Module3_Script_Bundle_v0.2.docx");
