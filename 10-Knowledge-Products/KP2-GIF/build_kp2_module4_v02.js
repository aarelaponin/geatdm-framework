// Build KP2 Module 4 — Video Script Bundle v0.2
// v0.2 (12 Sep 2026): aligned to the 08-Interoperability source (Method Steps 5–6; Reference Model §3, §4, §6; Reference
//   Architecture §3.3). 4.1 says what event distribution is for and that it may be deferred. 4.2 adds timestamping and the signed
//   audit log as the legal-evidence basis. 4.3 adds the per-standard fields — binding date, transition period, conformance-test
//   approach. 4.8 states the rule that platform membership never, by itself, authorises access to any dataset.
// Government Interoperability Framework · Topic 4 — Architecture, standards, and the Giga end-to-end case
...
// Persona for KP2 Module 4 (Architect throughout — the technical build; register shift per references/register-transposition.md)
...
body.push(
  ...
  new Paragraph({ spacing: { before: 0, after: 200 },
    children: [new TextRun({ text: "Topic 4 — Architecture, standards, and the Giga end-to-end case",
  ...
  specTable([
    ...
    ["Topic runtime",       "Approximately 40 minutes across eight standalone videos"],
...
body.push(
  ...
  P("Eight standalone subtopic videos. One Architect persona throughout. Total runtime approximately forty minutes. Each video has a single message and a single learning outcome, and is discoverable individually via search; the playlist provides navigation but is not required to comprehend any single video."),
  genericTable([700, 2700, 4700, 1600], ["#", "Title", "Single message", "Runtime"], [
    ["4.1", "Place every component — the four functional layers",
      "Four functional layers — Service Access, Event Distribution, Trust and Security, Governance and Administration — give every component of the platform a place.", "~5 min"],
    ["4.2", "Secure every call — the three trust zones",
      "Public, Member-Internal, Trust-Anchor — knowing which zone a call crosses tells you exactly what security it needs.", "~5 min"],
    ["4.3", "Adopt the standards portfolio",
      "Adopt the published standards — REST/OpenAPI, OAuth/OIDC, mTLS, X-Road — instead of writing your own; the portfolio is the menu every member shares.", "~5 min"],
    ["4.4", "Generate the semantic map",
      "Generate a semantic map so two agencies mean the same 'learner' before they exchange one — the hardest layer, made tractable.", "~5 min"],
    ["4.5", "Generate a service contract",
      "Turn a service brief into an OpenAPI contract, then an X-Road service description — the configuration that puts a service on the bus.", "~5 min"],
    ["4.6", "Put a real data source on the bus — the Giga case",
      "Take Giga's real school data through a bronze/silver/gold pipeline onto the bus — a worked exchange you can copy for your sector.", "~5 min"],
    ["4.7", "Wire a service onto the bus",
      "The OpenAPI contract becomes an X-Road service description and the call resolves — the GovStack Information Mediation pattern.", "~5 min"],
    ["4.8", "Make the exchange lawful — the data-protection envelope",
      "Letters of Interest plus a data-protection envelope make a real exchange lawful as well as technically possible.", "~5 min"]
  ]),
  pageBreak()
);
...
body.push(...renderSubtopic({
  ...
  slideSpecRows: [
    ...
    ["6", "Sources slide. Footer: 'Find the link in the description.'",
      "Lets viewers verify the reference architecture."]
  ],
  aiTip: {
    title: "Map your platform components to the four functional layers",
    problem: "An architect planning the interoperability platform needs to place every component into the four-layer reference architecture and find the gaps. This prompt produces that mapping.",
    ...
    io: "Input: the planned platform components. Output: a component-to-layer mapping plus a per-layer list of gaps.",
...
body.push(...renderSubtopic({
  ...
  title: "Secure every call — the three trust zones",
  runtime: "~5 min",
  words: 620,
  paeraAnchor: "EU EIF / NIIS X-Road trust model (three zones); mTLS; the X-Road message protocol",
  singleMessage: "Public, Member-Internal, Trust-Anchor — knowing which zone a call crosses tells you exactly what security it needs.",
  scriptBeats: [
    { cue: "Slide 1 — Title: 'Secure every call — the three trust zones'. Voice-over begins." },
    { text: "Security on an interoperability bus is not one wall around everything. It is zones, and the security a call needs depends on which zone boundaries it crosses. The X-Road trust model uses three zones. Once you know them, you can look at any exchange and state, precisely, what protects it — which is exactly the conversation a security reviewer or a data-protection officer will want to have with you." },
    { cue: "Slide 2 — Title: 'Three trust zones'. Body, three text rows: 'Public — the open internet; nothing trusted by default.' 'Member-Internal — inside an agency's own network; trusted by that agency.' 'Trust-Anchor — the certification authority everyone trusts to vouch for identities.'" },
    { text: "The three zones. Public — the open internet, where nothing is trusted by default. Member-Internal — inside an agency's own network, where that agency trusts its own systems. And the Trust-Anchor — the certification authority that everyone in the federation trusts to vouch for who is who. A message from one agency to another starts in one member's internal zone, crosses the public zone, and arrives in another member's internal zone — with the Trust-Anchor underwriting the identities at both ends." },
    { cue: "Slide 3 — Title: 'What protects each crossing'. Body, three text rows: 'Between security servers: mutual TLS — both sides prove who they are.' 'The message itself is signed and logged — the X-Road message protocol.' 'The Trust-Anchor issues and can revoke the certificates.' 'The log is timestamped and itself signed — it is the evidence the legal layer relies on.'" },
    { text: "Now the security falls out of the zones. When a message crosses the public zone between two security servers, it is protected by mutual TLS — meaning both sides present a certificate and prove who they are, not just one side as on an ordinary website. The message itself is signed and logged using the X-Road message protocol, so it cannot be forged, and neither side can later deny sending or receiving it. And the Trust-Anchor — the certification authority — issues those certificates and can revoke a compromised one, which is precisely how a misbehaving member gets cut off from the bus. And one more thing the security server does that is easy to overlook: every call is logged with its correlation identifier, sender, recipient, service and signature, and the log itself is timestamped and signed at intervals so it cannot be quietly altered. That tamper-evident log is not an operations nicety. It is the evidence base of the legal layer — the record a court, an auditor or a data-protection authority reads when an exchange is challenged. The decree can make an electronic exchange legally binding only because this log exists." },
    { cue: "Slide 4 — Title: 'The security server carries the trust burden'. Body, single text block: 'A security server sits at each member's edge and does this work — it is the member's gateway into the federation. The member's own systems stay in their internal zone and never face the public zone directly.'" },
    { text: "One device makes this practical: the security server, which sits at each member's edge and carries the trust burden. It is the member's gateway into the federation — it holds the certificates, does the mutual TLS, signs and logs the messages. The member's own systems stay safely in their internal zone and never face the public zone directly. That separation is exactly what lets a cautious agency join the bus without exposing its internal systems to the open internet — which is often the deciding factor in whether an agency will join at all." },
    { cue: "Slide 5 — Title: 'Trace an exchange across the zones'. Body, single text block: 'A call from the examination authority to the identity authority crosses one internal zone, the public zone, and another internal zone — so it needs mutual TLS, a signed message, and valid Trust-Anchor certificates at both ends. Say that, and you have specified its security.'" },
    { text: "The skill to take away is tracing any planned exchange across the zones and reading off its requirements. A call from the examination authority to the identity authority crosses from one internal zone, through the public zone, to another internal zone — so it needs mutual TLS between the two security servers, a signed and logged message, and valid certificates from the Trust-Anchor at both ends. Say exactly that, and you have specified the security of the exchange in terms any reviewer will accept." },
    { cue: "Slide 6 — Title: 'In one sentence'. Body, large text (Arial Bold 28pt): 'Name the zones a call crosses — public, member-internal, trust-anchor — and its security requirements are specified, not guessed.'" },
    { text: "So security on the bus is zoned, not uniform. Three zones — public, member-internal, trust-anchor. The security server carries the trust burden at each member's edge. And the discipline is to trace each exchange across the zones, because that is what turns 'is it secure?' from a worried question into a specified answer." },
    { cue: "Slide 7 — Title: 'Sources'. Body: EU EIF / NIIS X-Road trust model (three zones); mTLS; the X-Road message protocol; RFC 3161 time-stamping. Footer: 'Find the link in the description.'" }
  ],
  slideSpecRows: [
    ["1", "Title slide. Title: 'Secure every call — the three trust zones'.",
      "Standard ITU template. No images."],
    ["2", "Three-zones slide. Three text rows naming the zones.",
      "The model. Text-only."],
    ["3", "What-protects-each-crossing slide. Four text rows: mutual TLS, signed messages, the Trust-Anchor.",
      "Derives security from zones. Technical terms glossed in plain words."],
    ["4", "Security-server slide. Single text block on the edge gateway.",
      "The practical device. Explains why agencies can join safely."],
    ["5", "Trace-an-exchange slide. Single text block walking the PNEA-to-PNIA call across zones.",
      "The take-home skill, worked on a Progressa exchange."],
    ...
    ["7", "Sources slide. Footer: 'Find the link in the description.'",
      "Lets viewers verify the trust-model references."]
  ],
  aiTip: {
    title: "Trace an exchange across the trust zones and name its security",
    problem: "An architect specifying a cross-agency exchange needs to state its security requirements precisely, by tracing the zones it crosses. This prompt produces that trust-zone analysis.",
    ...
    io: "Input: a description of one cross-agency exchange. Output: a zone-by-zone security specification plus failure modes.",
...
body.push(...renderSubtopic({
  ...
  title: "Adopt the standards portfolio",
  runtime: "~5 min",
  words: 620,
  paeraAnchor: "Technical standards (REST / OpenAPI 3.x, OAuth 2.x / OIDC, mTLS, X-Road); semantic standards (ISO/IEC 11179, JSON-LD, W3C VC); EU EIF",
  singleMessage: "Adopt the published standards — REST/OpenAPI, OAuth/OIDC, mTLS, X-Road — instead of writing your own; the portfolio is the menu every member shares.",
  scriptBeats: [
    { cue: "Slide 1 — Title: 'Adopt the standards portfolio'. Voice-over begins." },
    { text: "The fastest way to break interoperability is to let each agency choose its own technical standards. The framework's answer is a standards portfolio — a single, published menu of the standards every member uses. As an architect, your job here is not to invent these. It is to select from the published menu, record your choices and their versions, and hold every member to them." },
    { cue: "Slide 2 — Title: 'The technical standards'. Body, four text rows: 'Describing services — REST with OpenAPI 3.x.' 'Proving identity and permissions — OAuth 2.x / OpenID Connect.' 'Securing the connection — mutual TLS.' 'The bus itself — the X-Road message protocol.'" },
    { text: "The technical standards, in plain terms. For how a service describes and offers itself — REST with OpenAPI 3.x, the common way to define a web service. For proving who a user or system is and what they are allowed to do — OAuth 2.x and OpenID Connect. For securing the connection — mutual TLS, where both ends prove their identity. And for the bus itself — the X-Road message protocol. None of these is invented for your country; each is a widely used published standard, with tools, documentation and people who already know it." },
    { cue: "Slide 3 — Title: 'The semantic standards'. Body, three text rows: 'Describing data elements — ISO/IEC 11179.' 'Linking data so meaning travels — JSON-LD.' 'Credentials a holder carries — W3C Verifiable Credentials.'" },
    { text: "Alongside the technical standards sit the semantic ones — the standards for meaning. ISO/IEC 11179 for describing data elements consistently. JSON-LD for linking data so its meaning travels with it. And W3C Verifiable Credentials for credentials a holder can carry and a verifier can trust without phoning the issuer. These are the published vocabularies you reuse rather than invent." },
    { text: "The whole point of a portfolio is re-use across the whole of government. A standard chosen once, for the framework, is reused by every member and by every new sector that joins — so the second agency does not re-decide how a service is described, and the education work you do now is reused when health comes onto the bus. Interoperability is, at bottom, the discipline of everyone choosing the same published standards instead of each building their own — and only whole-of-government planning can impose that discipline. A procurement clause cannot." },
    { cue: "Slide 4 — Title: 'Your deliverable — the portfolio document'. Body, single text block: 'A short document that names, for each need, the chosen standard, its version, and any profile (your small local refinements) — plus, per standard, the binding date, the transition period for legacy systems, and how conformance is tested. The APIs Working Group maintains it; every member conforms to it. Name the version — a standard at the wrong version is a quiet incompatibility.'" },
    ...
    { cue: "Slide 5 — Title: 'In one sentence'. Body, large text (Arial Bold 28pt): 'Select from the published menu, record the standard and its version, and hold every member to the same portfolio.'" },
    { text: "So the standards portfolio is the agreed menu, not a free choice. Select the technical and semantic standards from the published options, write them down with versions and any thin profile, and hold every member to the same list. That shared, reused portfolio is what lets twenty agencies behave as one framework instead of twenty incompatible projects." },
    { cue: "Slide 6 — Title: 'Sources'. Body: REST / OpenAPI 3.x; OAuth 2.x / OIDC; mTLS; X-Road; ISO/IEC 11179; JSON-LD; W3C Verifiable Credentials; EU EIF. Footer: 'Find the link in the description.'" }
  ],
  slideSpecRows: [
    ["1", "Title slide. Title: 'Adopt the standards portfolio'.",
      "Standard ITU template. No images."],
    ["2", "Technical-standards slide. Four text rows mapping need to standard.",
      "Each standard glossed in plain words. Text-only."],
    ["3", "Semantic-standards slide. Three text rows.",
      "Text-only."],
    ["4", "Portfolio-document slide. Single text block on the deliverable, with the version and thin-profile cautions.",
      "The practical artefact. Carries the re-use point in the body."],
    ["5", "Single-sentence summary slide. One large text block (Arial Bold 28pt).",
      "The take-home line."],
    ["6", "Sources slide. Footer: 'Find the link in the description.'",
      "Lets viewers verify the standards."]
  ],
  aiTip: {
    title: "Assemble your standards portfolio from the published menu",
    problem: "An architect needs to assemble the framework's standards portfolio — the chosen standard and version for each need — from the published options, with thin local profiles. This prompt drafts it.",
    ...
    io: "Input: the country (and any standards already in use). Output: a standards-portfolio table — standard, version, profile, binding date, transition period, conformance-test approach — plus the decisions that need deliberation.",
...
body.push(...renderSubtopic({
  ...
  title: "Generate the semantic map",
  runtime: "~5 min",
  words: 600,
  paeraAnchor: "ISO/IEC 11179; JSON-LD; W3C VC; education vocabularies — OneRoster (1EdTech), CEDS; EU EIF semantic layer",
  singleMessage: "Generate a semantic map so two agencies mean the same 'learner' before they exchange one — the hardest layer, made tractable.",
  scriptBeats: [
    { cue: "Slide 1 — Title: 'Generate the semantic map'. Voice-over begins." },
    { text: "Of the four interoperability layers, the semantic one — meaning — is the hardest, and the one most projects skip, because it is slow, unglamorous and a little political. It is also where exchanges silently fail: a working wire that carries data the receiver misreads is still a failure, and a worse one, because it looks like success. The good news for the architect is that meaning can be made tractable by building a semantic map — and you can generate a first draft of one with Claude." },
    { cue: "Slide 2 — Title: 'What a semantic map is'. Body, five text rows: 'The entities in the exchange — learner, school.' 'Their fields — name, date of birth, enrolment status.' 'The code lists — the allowed values.' 'The identifier — which key links the same learner across two agencies.' 'The mapping — each agency's terms onto one shared definition.'" },
    { text: "A semantic map, for a single exchange, sets out five things. The entities involved — a learner, a school. Their fields — name, date of birth, enrolment status. The code lists — the allowed values for, say, enrolment status, because one agency's 'active' may be another's 'enrolled'. The identifier — which key reliably links the same learner across two agencies. And the mapping itself — each agency's own terms reconciled to one shared definition, so that the examination authority's 'learner' and the registry's 'learner' are confirmed to mean the same person, identified the same way." },
    { cue: "Slide 3 — Title: 'Reuse the published vocabularies'. Body, three text rows: 'OneRoster — rostering and enrolment data.' 'CEDS — a common vocabulary of education data elements.' 'ISO/IEC 11179 — how to describe a data element precisely; W3C VC for credentials.'" },
    { text: "You do not invent these definitions from scratch. For education, much of the meaning is already published. OneRoster gives you rostering and enrolment data. CEDS gives you a common vocabulary of education data elements. ISO/IEC 11179 gives you the discipline of describing each data element precisely, and W3C Verifiable Credentials covers the credential case. The semantic map's job is to adopt these published vocabularies and map your agencies' actual fields onto them — not to start from a blank page." },
    { cue: "Slide 4 — Title: 'Where business and IT meet'. Body, single text block: 'Deciding that 'enrolment' means the same to the examination authority and the learner registry is a decision the data owners make — but it must be written precisely enough for the systems to use. The semantic map is that shared language: agreed once, reused by every exchange.'" },
    { text: "This is the layer where business and IT must meet, and the semantic map is their shared language. Deciding that 'enrolment' means the same thing to the examination authority and to the learner registry is not a technical choice — it is a decision the business data owners make. But it has to be written down precisely enough for the systems to use. The semantic map is exactly that shared object: it lets the people who own the data and the architects who move it agree, once, what the data means — and then every exchange that touches a learner reuses that one agreement instead of re-arguing it." },
    { cue: "Slide 5 — Title: 'Generate the semantic map, then confirm'. Body, two text rows: 'Give Claude the published vocabulary and the two agencies' field lists; it drafts the mapping.' 'Every identifier and code value is a [confirm] until checked against the real registries — an invented mapping is confident and wrong.'" },
    { text: "The AI prompt for this video does the drafting. Give Claude the published vocabulary and the two agencies' field lists, and it drafts the mapping — which field maps to which, where the code lists differ and need translating, which identifier to use as the link. That is the fast ninety per cent. The essential ten per cent is yours: every identifier and every code value is a [confirm] until you check it against the real registries. An invented mapping reads convincingly and is wrong — and in the semantic layer, wrong means a learner's records silently merged with someone else's. Generate fast; confirm against the registries carefully." },
    { cue: "Slide 6 — Title: 'In one sentence'. Body, large text (Arial Bold 28pt): 'A semantic map is the shared meaning of an exchange — drafted by AI from the published vocabularies, confirmed against the real registries.'" },
    { text: "So you make the hardest layer tractable by building it, one exchange at a time, as a semantic map: the entities, fields, code lists, identifier and the mapping onto published vocabularies. It is the shared language between the data owners and the architects, generated with the AI prompt and confirmed against the registries. Get the semantic map right and the exchange carries meaning, not just bytes — which is the whole difference between interoperability that works and interoperability that only appears to." },
    { cue: "Slide 7 — Title: 'Sources'. Body: ISO/IEC 11179; JSON-LD; W3C Verifiable Credentials; OneRoster (1EdTech); CEDS; EU EIF semantic layer. Footer: 'Find the link in the description.'" }
  ],
  slideSpecRows: [
    ["1", "Title slide. Title: 'Generate the semantic map'.",
      "Standard ITU template. No images."],
    ["2", "What-a-semantic-map-is slide. Five text rows: entities, fields, code lists, identifier, mapping.",
      "The artefact anatomy. Text-only."],
    ["3", "Reuse-vocabularies slide. Three text rows: OneRoster, CEDS, ISO 11179 / W3C VC.",
      "Names the published education vocabularies. No logos, plain text."],
    ["4", "Business-and-IT slide. Single text block on meaning as a business decision written for systems.",
      "Carries the lingua-franca argument. Text-only."],
    ["5", "Generate-then-confirm slide. Two text rows: generate, then [confirm] against registries.",
      "Names the AI play and the anti-invention safeguard."],
    ["6", "Single-sentence summary slide. One large text block (Arial Bold 28pt).",
      "The take-home line."],
    ["7", "Sources slide. Footer: 'Find the link in the description.'",
      "Lets viewers verify the semantic standards and vocabularies."]
  ],
  aiTip: {
    title: "Generate a semantic map for an exchange",
    problem: "An architect needs a semantic map for one exchange — the entities, fields, code lists, identifier and the mapping onto published vocabularies — drafted from the two agencies' field lists. This is the semantic-map play.",
    ...
    io: "Input: two agencies' field lists for a shared entity, and the published vocabulary to align to. Output: a semantic map with [confirm] placeholders for identifiers and code values.",
  ...
  metadataRows: [
    ...
    ["Description (60 words)", "Meaning is the hardest interoperability layer and the one projects skip — a working wire carrying misread data is still a failure. A semantic map fixes it: entities, fields, code lists, the identifier, and the mapping onto published vocabularies like OneRoster and CEDS. It's the shared language between data owners and architects. Five minutes for architects. The AI prompt is in the description."],
...
body.push(...renderSubtopic({
  ...
  title: "Generate a service contract",
  runtime: "~5 min",
  words: 580,
  paeraAnchor: "OpenAPI 3.x specification; OAuth 2.x / OIDC; NIIS X-Road service description; EU EIF",
  singleMessage: "Turn a service brief into an OpenAPI contract, then an X-Road service description — the configuration that puts a service on the bus.",
  scriptBeats: [
    { cue: "Slide 1 — Title: 'Generate a service contract'. Voice-over begins." },
    { text: "To put a service on the bus, you need a contract — a precise description of what the service offers: the operations, the inputs, the outputs, the errors. That contract is an OpenAPI document, and it is the configuration that makes the service callable. As an architect, you generate it from a short service brief and the semantic map, and then turn it into the X-Road service description the bus actually uses to route a call." },
    { cue: "Slide 2 — Title: 'What an OpenAPI contract specifies'. Body, five text rows: 'The operations — e.g. get a learner record.' 'The inputs — the identifier and parameters.' 'The outputs — the fields, taken from the semantic map.' 'The errors it can return.' 'The security — OAuth 2.x / OpenID Connect.'" },
    { text: "An OpenAPI 3.x contract specifies, for one service, the operations it offers — say, 'get a learner's record' — the inputs each operation takes, the outputs it returns, the errors it can raise, and the security it requires. The outputs come straight from the semantic map you built: the contract is where the agreed meaning becomes a concrete, callable interface. A consumer reads the contract and knows exactly how to call the service — the field names, the types, the errors — without a single meeting." },
    { cue: "Slide 3 — Title: 'Generate the contract, then confirm'. Body, two text rows: 'Give Claude the service brief and the semantic map; it drafts the OpenAPI document — paths, request and response shapes, errors, security.' 'Every endpoint, field and type is a [confirm] until checked — a contract naming a field the provider does not expose fails at the first call.'" },
    { text: "The AI prompt for this video does the drafting. Give Claude the service brief — what the service does — and the semantic map for the data it returns, and it drafts the OpenAPI document: the paths, the request and response shapes, the error responses, the security scheme. As with the semantic map, every endpoint, field name and type is a [confirm] until you check it against what the provider system actually exposes. An OpenAPI document that looks right but names a field the provider does not return is a contract that fails at the first real call — and looks fine until then." },
    { cue: "Slide 4 — Title: 'One contract, two forms'. Body, single text block: 'The OpenAPI contract becomes the X-Road service description — the form the bus uses to register and route to the service. OpenAPI for humans and tools to read; the X-Road service description for the bus to route. Generate the first, derive the second.'" },
    { text: "The OpenAPI contract then becomes the X-Road service description — the form the bus uses to register the service and route a call to it. They are the same contract in two forms: OpenAPI for humans and tools to read, the X-Road service description for the bus to route. Generating the first and deriving the second is the core move of this whole knowledge product — using Claude to produce the configuration that puts a real service on the bus." },
    { text: "And this is why the contract is a build-pack artefact, not a diagram. It is executable configuration — it is what the consumer's code calls and what the bus routes. When you have done the semantic map and the service contract for one exchange, you hold, for that exchange, the agreed meaning and the callable interface: the technical-layer configuration the live demonstration runs on. That is real output, not a picture of output." },
    { cue: "Slide 5 — Title: 'In one sentence'. Body, large text (Arial Bold 28pt): 'The OpenAPI contract is the service made callable — generated from the brief and the semantic map, confirmed, and turned into an X-Road service description.'" },
    { text: "So the service contract is where meaning becomes a callable interface. Generate the OpenAPI document with the AI prompt from the brief and the semantic map, confirm every field against the provider, and derive the X-Road service description. That is the configuration that puts a service on the bus — executable, in the build pack, ready for a real call." },
    { cue: "Slide 6 — Title: 'Sources'. Body: OpenAPI 3.x specification; OAuth 2.x / OpenID Connect; NIIS X-Road service description; EU EIF. Footer: 'Find the link in the description.'" }
  ],
  slideSpecRows: [
    ["1", "Title slide. Title: 'Generate a service contract'.",
      "Standard ITU template. No images."],
    ["2", "OpenAPI-specifies slide. Five text rows: operations, inputs, outputs, errors, security.",
      "The contract anatomy. Text-only."],
    ["3", "Generate-then-confirm slide. Two text rows: generate; [confirm] every field.",
      "Names the AI play and the anti-invention safeguard."],
    ["4", "One-contract-two-forms slide. Single text block: OpenAPI and the X-Road service description.",
      "The core 'config that wires a service' move. Text-only."],
    ["5", "Single-sentence summary slide. One large text block (Arial Bold 28pt).",
      "The take-home line."],
    ["6", "Sources slide. Footer: 'Find the link in the description.'",
      "Lets viewers verify the OpenAPI and X-Road references."]
  ],
  aiTip: {
    title: "Generate an OpenAPI service contract",
    problem: "An architect needs an OpenAPI 3.x contract for a service, generated from the service brief and the semantic map, ready to derive an X-Road service description. This is the service-contract play.",
    ...
    io: "Input: the service brief and the relevant semantic-map fields. Output: an OpenAPI 3.x contract with [confirm] placeholders, plus X-Road service-description notes.",
...
body.push(...renderSubtopic({
  ...
  title: "Put a real data source on the bus — the Giga case",
  runtime: "~5 min",
  words: 580,
  paeraAnchor: "Giga open APIs and schemas (school-master, qos); GeoJSON; ISO 3166-1 alpha-3; bronze/silver/gold MDM",
  singleMessage: "Take Giga's real school data through a bronze/silver/gold pipeline onto the bus — a worked exchange you can copy for your sector.",
  scriptBeats: [
    { cue: "Slide 1 — Title: 'Put a real data source on the bus — the Giga case'. Voice-over begins." },
    { text: "Everything so far has been the method. Now a real worked case, so you have a template to copy rather than a theory to apply. Giga — the ITU and UNICEF initiative that maps the world's schools and their connectivity — publishes open APIs and real school data. Putting Giga's data onto the bus is a complete, concrete example of every step at once: the standards, the semantics, the contract, and the data pipeline that feeds them." },
    { cue: "Slide 2 — Title: 'What Giga gives you'. Body, four text rows: 'Open APIs you can call.' 'A school-master schema and a qos (connectivity) schema.' 'GeoJSON for each school's location.' 'ISO 3166-1 alpha-3 country codes for indexing.'" },
    { text: "Giga gives you real, published material to work from. Open APIs you can actually call. A school-master schema describing each school, and a qos schema describing its connectivity — the quality of its internet service. GeoJSON, the standard way to express a location, for where each school sits. And ISO 3166-1 alpha-3, the three-letter country codes, for indexing. These are exactly the kind of published standards and schemas the standards portfolio and the semantic map told you to reuse — and here they are, in one real dataset, ready to practise on." },
    { cue: "Slide 3 — Title: 'The bronze / silver / gold pipeline'. Body, three text rows: 'Bronze — the raw data, exactly as received; kept for audit.' 'Silver — cleaned and validated, conformed to the schema and the semantic map; bad records flagged.' 'Gold — the published, authoritative version other services consume.'" },
    { text: "Real source data is never clean enough to publish straight onto the bus, so it moves through a pipeline in three stages. Bronze — the raw data exactly as received from the source, kept untouched for audit. Silver — the cleaned and validated version, conformed to the schema and the semantic map, with bad or duplicate records flagged. Gold — the published, authoritative version that other services on the bus actually consume. This bronze, silver, gold pattern is how a messy real source becomes a registry other agencies can trust, and every data source you put on the bus follows it." },
    { cue: "Slide 4 — Title: 'The whole module, on one source'. Body, single text block: 'School data lands as bronze, is conformed to the schema and semantic map as silver, and is published as gold. A gold service gets an OpenAPI contract, an X-Road service description, trust-zone security, and a data-protection envelope. That is Module 4, applied end to end.'" },
    { text: "Now watch the whole module tie together on one source. The school-master data lands as bronze. It is cleaned and conformed to the schema and the semantic map as silver. It is published as gold. The gold-layer service is described by an OpenAPI contract, registered as an X-Road service description, secured across the three trust zones, and made lawful by the data-protection envelope. Every piece of this module appears, in order, on one real dataset. That is the template — and it is the same template whether the source is schools, health facilities, or a farmer registry." },
    { text: "The reason to learn it on Giga rather than your own data is that Giga is public, real and already standards-aligned, so you can practise the full pipeline before you touch a live national registry where a mistake has consequences. Once you have run it once on Giga, you apply the identical pattern — bronze, silver, gold, contract, bus — to your country's own school data, and then to the next sector." },
    { cue: "Slide 5 — Title: 'In one sentence'. Body, large text (Arial Bold 28pt): 'Bronze, silver, gold, contract, bus — the Giga case is the full method on one real source, ready to copy for your own sector.'" },
    { text: "So the Giga case is the method made concrete. A real source, real schemas, the bronze-silver-gold pipeline, a service contract, and the bus — every step of this module on one dataset you can actually run. Learn it here, where the data is public and forgiving, and you have a template to copy onto your own national registries with confidence." },
    { cue: "Slide 6 — Title: 'Sources'. Body: Giga open APIs and schemas (school-master, qos); GeoJSON; ISO 3166-1 alpha-3; bronze/silver/gold MDM (Giga School Master Data architecture). Footer: 'Find the link in the description.'" }
  ],
  slideSpecRows: [
    ["1", "Title slide. Title: 'Put a real data source on the bus — the Giga case'.",
      "Standard ITU template. No images."],
    ["2", "What-Giga-gives slide. Four text rows: APIs, schemas, GeoJSON, country codes.",
      "Names the real Giga assets. No logos, plain text."],
    ["3", "Bronze-silver-gold slide. Three text rows for the three pipeline stages.",
      "The MDM pattern. Text-only."],
    ["4", "Whole-module-on-one-source slide. Single text block tying every step together.",
      "The synthesis — the template the architect copies."],
    ["5", "Single-sentence summary slide. One large text block (Arial Bold 28pt).",
      "The take-home line."],
    ["6", "Sources slide. Footer: 'Find the link in the description.'",
      "Lets viewers verify the Giga assets and the MDM pattern."]
  ],
  aiTip: {
    title: "Map a real sector data source through bronze/silver/gold onto the bus",
    problem: "An architect needs to plan how a real source dataset (Giga schools, or the country's own sector registry) moves through a bronze/silver/gold pipeline to a gold service on the bus. This prompt produces that plan.",
    ...
    io: "Input: a description of the source dataset. Output: a bronze/silver/gold pipeline plan plus a path-onto-the-bus checklist.",
...
body.push(...renderSubtopic({
  ...
  title: "Wire a service onto the bus",
  runtime: "~5 min",
  words: 540,
  paeraAnchor: "NIIS X-Road service description / Information Mediator; GovStack Information Mediation Building Block; EU EIF",
  singleMessage: "The OpenAPI contract becomes an X-Road service description and the call resolves — the GovStack Information Mediation pattern.",
  scriptBeats: [
    { cue: "Slide 1 — Title: 'Wire a service onto the bus'. Voice-over begins." },
    { text: "You have the meaning — the semantic map. The contract — the OpenAPI document. And the data — the gold dataset. Wiring is the step that makes them callable on the bus: registering the service so that a consumer in another agency can discover it and call it. The pattern has a name in the GovStack world — Information Mediation, the building block whose whole job is to mediate exchanges between systems." },
    { cue: "Slide 2 — Title: 'What 'wired' means'. Body, three text rows: 'The service description is registered on the provider's security server.' 'A consumer in another agency can discover the service.' 'A consumer call routes through both security servers and returns the data.'" },
    { text: "'Wired' means three concrete things. The service's description is registered on the provider agency's security server. A consumer in another agency can discover that the service exists. And when the consumer calls it, the request routes through the consumer's security server, across the bus, to the provider's security server, which runs the service and returns the data — all secured across the trust zones you mapped earlier. When that round trip works, the service is genuinely on the bus, not just designed for it." },
    { cue: "Slide 3 — Title: 'The configuration that makes it work'. Body, three text rows: 'The subsystem — the member's registered identity on the bus.' 'The service description — derived from your OpenAPI contract.' 'The access-control list — which members may call this service.'" },
    { text: "Three configuration artefacts make wiring real, and you have largely produced them already. The subsystem — the member's registered identity on the bus. The service description — derived directly from your OpenAPI contract. And the access-control list — which other members are allowed to call this service, because being on the bus does not mean everyone may call everything. Registering the member as a subsystem and setting its access-control list is member-onboarding work; here you supply the service description that goes with it." },
    { cue: "Slide 4 — Title: 'The GovStack Information Mediation pattern'. Body, single text block: 'Information Mediation is the published GovStack building block for the component that routes and mediates exchanges. Cross-linking to it keeps your framework aligned with the wider ecosystem — a vendor's compliant Information Mediator drops into the role rather than being built bespoke.'" },
    ...
    { cue: "Slide 5 — Title: 'The acceptance is a test call'. Body, single text block: 'A consumer security server calls the service and gets the expected data back, once, over the bus. That single resolving call proves the technical layer works for this exchange: meaning agreed, contract honoured, security enforced, data returned. It is exactly the check the live demonstration runs.'" },
    { text: "And the acceptance here is concrete and runnable — a test call. A consumer's security server calls the service and gets the expected data back, once, over the bus. That single resolving call is the proof that the technical layer works for this exchange: meaning agreed, contract honoured, security enforced, data returned. It is exactly the check the live demonstration runs, and it is the moment the technical-layer configuration stops being a set of documents and becomes a working exchange." },
    { cue: "Slide 6 — Title: 'In one sentence'. Body, large text (Arial Bold 28pt): 'Register the service description, set who may call it, and a test call that resolves proves the service is on the bus.'" },
    { text: "So wiring is where the pieces become a working service. Register the service description on the security server, set the access-control list for who may call it, align to the GovStack Information Mediation pattern, and prove it with a test call that resolves. That resolving call is the technical layer's acceptance — and the technical configuration the framework actually runs on." },
    { cue: "Slide 7 — Title: 'Sources'. Body: NIIS X-Road service description / Information Mediator; GovStack Information Mediation Building Block (govstack.global); EU EIF. Footer: 'Find the link in the description.'" }
  ],
  slideSpecRows: [
    ["1", "Title slide. Title: 'Wire a service onto the bus'.",
      "Standard ITU template. No images."],
    ["2", "What-wired-means slide. Three text rows: register, discover, route-and-return.",
      "Defines 'wired' concretely. Text-only."],
    ["3", "Configuration slide. Three text rows: subsystem, service description, access-control list.",
      "The config artefacts. Links to member onboarding (Module 5). Text-only."],
    ["4", "Information-Mediation slide. Single text block on the GovStack BB pattern.",
      "Cross-links GovStack; carries re-use to the routing component."],
    ["5", "Test-call slide. Single text block on the resolving call as acceptance.",
      "The runnable acceptance — the demonstration's check."],
    ["6", "Single-sentence summary slide. One large text block (Arial Bold 28pt).",
      "The take-home line."],
    ["7", "Sources slide. Footer: 'Find the link in the description.'",
      "Lets viewers verify the X-Road and GovStack references."]
  ],
  aiTip: {
    title: "Generate the X-Road service description and wiring checklist",
    problem: "An architect needs to turn an OpenAPI contract into the X-Road service description and the wiring configuration (subsystem, access-control list), with a test-call plan. This prompt produces them.",
    ...
    io: "Input: the OpenAPI contract, the provider and consumer agencies, the access policy. Output: X-Road service-description notes, access-control entries, and a test-call plan.",
...
body.push(...renderSubtopic({
  ...
  title: "Make the exchange lawful — the data-protection envelope",
  runtime: "~5 min",
  words: 540,
  paeraAnchor: "ITU DPI Safeguards / data-protection guidance; bilateral Letters of Interest; the Topic-2 decree; EU EIF legal layer",
  singleMessage: "Letters of Interest plus a data-protection envelope make a real exchange lawful as well as technically possible.",
  scriptBeats: [
    { cue: "Slide 1 — Title: 'Make the exchange lawful — the data-protection envelope'. Voice-over begins." },
    { text: "A service can be perfectly wired and perfectly secured and still must not be switched on, because technical capability is not lawful authority. The last thing the architect checks for an exchange is its data-protection envelope — the agreements and safeguards that make moving this data lawful, not merely possible. This is where the technical work you have done meets the legal and organisational layers built earlier in this knowledge product." },
    { cue: "Slide 2 — Title: 'The envelope has three parts'. Body, three text rows: 'A lawful basis — from the decree; the exchange must be one the decree authorises.' 'A bilateral Letter of Interest — the two agencies agree to this specific exchange, on these terms.' 'Data-protection safeguards — minimisation, purpose, consent where needed, retention.'" },
    ...
    { cue: "Slide 3 — Title: 'Data protection by design'. Body, three text rows: 'Minimisation is in the contract — the service returns only the fields the purpose needs.' 'Purpose and the access-control list match — only agencies with a lawful purpose can call.' 'Retention and logging are configured, not promised.'" },
    { text: "The architect's specific job is data protection by design — building the safeguards into the configuration rather than bolting them on after. Minimisation lives in the OpenAPI contract: the service returns only the fields the purpose needs, not the whole record. The purpose and the access-control list match: only the agencies with a lawful purpose can call the service. And retention and logging are configured, not merely promised in a policy. Built this way, the safeguards are part of the configuration the demonstration runs, not a document filed somewhere separate from the system." },
    { cue: "Slide 4 — Title: 'Where the three layers meet'. Body, single text block: 'The decree (legal) authorises the exchange. The member agreement and access-control list (organisational) record who may do it. The contract, security and minimisation (technical) carry it out within the envelope. An exchange is ready only when all three line up. And the rule underneath: membership of the bus never, by itself, grants access to any data — access is granted per service, by the data owner.'" },
    ...
    { cue: "Slide 5 — Title: 'In one sentence'. Body, large text (Arial Bold 28pt): 'A lawful basis, a Letter of Interest, and data protection by design — the envelope that makes a wired exchange a lawful one. Membership is never access.'" },
    ...
    { cue: "Slide 6 — Title: 'Sources'. Body: ITU DPI Safeguards / data-protection guidance; bilateral Letters of Interest; the Module 2 decree (legal basis); EU EIF legal layer. Footer: 'Find the link in the description.'" }
  ],
  slideSpecRows: [
    ["1", "Title slide. Title: 'Make the exchange lawful — the data-protection envelope'.",
      "Standard ITU template. No images."],
    ["2", "Three-parts slide. Three text rows: lawful basis, Letter of Interest, safeguards.",
      "The envelope anatomy. Text-only."],
    ["3", "Data-protection-by-design slide. Three text rows: minimisation, purpose match, retention configured.",
      "The architect's concrete job. Text-only."],
    ["4", "Three-layers-meet slide. Single text block tying legal, organisational, technical together.",
      "The synthesis across this knowledge product's layers. Text-only."],
    ["5", "Single-sentence summary slide. One large text block (Arial Bold 28pt).",
      "The take-home line that closes the module."],
    ["6", "Sources slide. Footer: 'Find the link in the description.'",
      "Lets viewers verify the data-protection references."]
  ],
  aiTip: {
    title: "Draft the data-protection envelope for an exchange",
    problem: "An architect needs the data-protection envelope for an exchange — the lawful basis, the Letter of Interest, and the by-design safeguards — assembled and checked against the decree. This prompt drafts it.",
    ...
    io: "Input: the exchange and the relevant decree articles. Output: a lawful-basis finding, a draft Letter of Interest, and a data-protection-by-design checklist.",
...
body.push(
  ...
  P("The v0.1 drafting (and the v0.2 alignment to the source method) raised the editorial and structural decisions below. These are forwarded for ITU's discussion at the Tuesday weekly call."),

  H3("5.1 Standards and schema claims to verify"),
  P("The technical and semantic standards named in this topic must be confirmed against their published specifications — version and currency — before final lock: the technical standards (REST / OpenAPI 3.x, OAuth 2.x / OIDC, mTLS, the X-Road message protocol) in 4.3 and 4.5; the semantic standards (ISO/IEC 11179, JSON-LD, W3C Verifiable Credentials) and the education vocabularies (OneRoster, CEDS) in 4.4; and the Giga assets (the school-master and qos schemas, GeoJSON, ISO 3166-1 alpha-3) plus the GovStack Information Mediation Building Block in 4.6 and 4.7. A standard cited at the wrong version is the kind of error a technical reviewer will catch."),

  H3("5.2 Editorial tone calls"),
  P("Sharp lines that deserve a deliberate keep / soften / cut decision: 'the semantic layer is the hardest layer, and the one most projects skip' (4.4); 'invented configuration reads convincingly and is worthless' (4.5); 'a working wire that carries the wrong meaning is still a failure' (4.4, echoing Topic 1)."),

  H3("5.3 The register shift (first Architect-facing topic)"),
...
body.push(
  ...
  P("Compiled across the eight subtopics for ITU's video production pipeline. To be split per subtopic and inserted into the corresponding YouTube descriptions."),
...
const doc = new Document({
  ...
  title: "KP2 Module 4 — Video Script Bundle v0.2 (ITU-aligned)",
  description: "Video script bundle for KP2 Topic 4 (Government Interoperability Framework), aligned to ITU's Knowledge Products and Video Materials Guide.",
  styles: {
  ...
  sections: [{
    ...
    headers: { default: new Header({ children: [new Paragraph({ alignment: AlignmentType.RIGHT,
      children: [new TextRun({ text: "FiscalAdmin OÜ — ITU/Giga · KP2 Topic 4 Script Bundle v0.2 · 12 September 2026",
...
Packer.toBuffer(doc).then(buf => {
  const out = process.env.OUT_PATH || path.join(__dirname, "KP2_Module4_Script_Bundle_v0.2.docx");
