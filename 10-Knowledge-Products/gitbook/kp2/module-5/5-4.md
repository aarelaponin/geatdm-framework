...
## Do this on your own sector — Generate the X-Road member registration (subsystem + ACL)
...
**What it does.** An architect onboarding an agency needs the X-Road subsystem registration and the access-control list generated from the member's details and the access policy — the config that admits the member. This is the bb-config-gen play for member onboarding.
