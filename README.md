...
|--------|-----------|---------|
| **08-Interoperability** | Reference Model (concept) + Method (8 steps) + **Toolkit** — 14 templates + the new **Reference Architecture (RA)**, **RA-to-RFP plugin (10 skills)** & guide | National Government Interoperability Framework — the substrate every sectoral architecture consumes for cross-government data exchange. A value chain: **concept** (Reference Model) → **process** (Method) → **enforceable target design** (RA) → **procurement** (the RA-to-RFP plugin turns the RA + a country's National EA into an issuable, funder-compliant tender). In the KP2 / Implementation-Plan structure the RA and plugin are **part of the Toolkit pillar**, so the published plan needs no change. See the conceptual model in `GEATDM-Interop-RA-to-RFP-Guide-v1.0.md`. PAERA §3.4.3 + §3.2 + §5.2 #2/#5/#9 |
